...
// La última vuelta baja por siete beats legibles. Se comparte con el arte de
// la cascada para que la pista física y la lectura visual no se desfasen.
export const CHORRERA_BEATS = [0.205, 0.218, 0.232, 0.247, 0.264, 0.282, 0.302];
export const CHORRERA_DROPS = [2.2, 3.6, 5.4, 2.8, 2.4, 4.8];
...
function perfil(f) {
  ...
  // Descenso de La Chorrera: seis labios con repisa entre ellos. El perfil
  // baja en escalones, de modo que el kart no atraviesa una cascada flotando
  // sobre una pista plana. El ancho de cada transición deja margen para que
  // el salto arcade existente siga siendo controlable.
  let chorreraDrop = 0;
  for (let i = 0; i < CHORRERA_DROPS.length; i++) {
    const t = CHORRERA_BEATS[i + 1];
    chorreraDrop += CHORRERA_DROPS[i] * suave((f - t) / 0.0045);
  }
  y -= chorreraDrop;
...
export function construirPista(opts = {}) {
  ...
  return {
    ...
    NOMBRE_ZONA,
    alturaMundo, normalMundo, infoLocal, puntoEn, perfil, minimapa: minPts,
