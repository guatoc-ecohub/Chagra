...
import { construirPista, ZONA, CHORRERA_BEATS } from './pista.js';
import { VEHICULOS, TURBO_NIVELES } from './vehiculos.js';
import { PILOTOS, ITEMS, PILOTO_POR_ID, PILOTOS_CASA, pilotoDesbloqueado, pilotoDesdeCompai } from './pilotos.js';
...
const camara = new THREE.PerspectiveCamera(62, innerWidth / innerHeight, 0.3, 900);
...
function montarCoche() {
  ...
  const chorreraStart = new URLSearchParams(location.search).get('chorreraStart') === '1';
  fisica = crearFisica(pista, vehCarrera, { totalLaps: 3, f0: chorreraStart ? 0.205 : 0.0 });
  rescatesVistos = 0;
  ...
  fauna = crearFauna(THREE, pista, { escena });
  construirItemsPista();
  fisica.vel = 0;
  fisicaInvitado = null;
  fxToon.limpiar();
...
const ent = crearEntorno(THREE, pista, {
...
const EXPOSICION_BASE = GATE_CHORRERA ? 0.82 : 1.05;
...
const GATE_CHORRERA_VIEW = (() => {
  const offsets = [-13, 11, -10, 14, -12, 10, -16];
  const point = (i) => {
    const q = pista.puntoEn(CHORRERA_BEATS[i]);
    const sx = Math.sin(q.hdg), sz = -Math.cos(q.hdg);
    const x = q.x + sx * offsets[i], z = q.z + sz * offsets[i];
    return { q, x, z, y: pista.alturaMundo(x, z) + 4 };
  };
  const dist = new URLSearchParams(location.search).get('chorreraDist') || 'media';
  const near = point(1), mid = point(3), far = point(6);
  const actionQ = pista.puntoEn(CHORRERA_BEATS[3]);
  const action = { x: actionQ.x, z: actionQ.z, y: pista.alturaMundo(actionQ.x, actionQ.z) + 4 };
  const presets = {
    accion: { back: 17, up: 10, side: -15, target: action, origin: action },
    cerca: { back: 30, up: 22, side: -28, target: mid },
    media: { back: 62, up: 38, side: -58, target: far },
    amplia: { back: 104, up: 68, side: -88, target: far },
  };
  const p = presets[dist] || presets.media;
  const aim = p.target;
  const origin = p.origin || near;
  return {
    cam: new THREE.Vector3(origin.x + p.side, origin.y + p.up, origin.z + p.back),
    mira: new THREE.Vector3(aim.x, aim.y - (dist === 'amplia' ? 3 : 1), aim.z - (dist === 'cerca' ? 10 : 24)),
    dist,
  };
})();
...
function asegurarProxyKart(k, dt) {
  ...
  if (!entry || entry.vehiculoId !== veh.id || entry.pilotoId !== (k.piloto || '')) {
    if (entry?.modelo?.grupo) escena.remove(entry.modelo.grupo);
    const modeloKart = construirModeloVehiculo(THREE, { ...veh, piloto: k.piloto || veh.piloto || 'angelita' });
...
function usarPoderJugador() {
  ...
  } else if (piloto.id === 'crisopa') {
    vfxOpts.anclas = [];
    for (const r of prox) {
      if (dist2(r.modelo?.grupo.position.x ?? 0, r.modelo?.grupo.position.z ?? 0) < 30 * 30) {
        apply(r, 'larvas', 1.6);
        if (r.pos) vfxOpts.anclas.push(r.pos);
      }
    }
  } else if (piloto.id === 'avispita') {
    const lead = prox.reduce((acc, r) => (acc && acc.f > r.f ? acc : r), null);
    if (lead) {
      apply(lead, 'freeze', 1.3);
      vfxOpts.objetivo = lead.pos ?? null;
    }
  } else if (piloto.id === 'mariquita') {
    carrera.hazards = carrera.hazards.filter((h) => (h.x - px) ** 2 + (h.z - pz) ** 2 > 8 * 8);
  } else if (piloto.id === 'momificador') {
    const front = prox.find((r) => ((r.f - (fisica.info?.f ?? 0) + 1) % 1) < 0.08);
    if (front) {
      apply(front, 'freeze', 1.8);
      vfxOpts.objetivo = front.pos ?? null;
    }
  } else if (piloto.id === 'murcielago') {
    carrera.oscuridad = 2.2;
  } else if (piloto.id === 'morpho') {
    for (const r of prox) if (dist2(r.modelo?.grupo.position.x ?? 0, r.modelo?.grupo.position.z ?? 0) < 26 * 26) apply(r, 'ceguera', 1.6);
  } else if (piloto.id === 'beauveria') {
    for (const r of prox) if (dist2(r.modelo?.grupo.position.x ?? 0, r.modelo?.grupo.position.z ?? 0) < 26 * 26) apply(r, 'esporas', 2.2);
  }
...
function cuadroKart() {
  ...
  camara.fov -= fovExtraRapido;
  if (GATE_CHORRERA) {
    camara.position.copy(GATE_CHORRERA_VIEW.cam);
    camara.lookAt(GATE_CHORRERA_VIEW.mira);
  } else {
    cam.actualizar(dt, fisica, {});
  }
  actualizarCamaraRapida(dt, fisica);
