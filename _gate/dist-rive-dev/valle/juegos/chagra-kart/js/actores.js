...
export function crearRivales(THREE, pista, cfg = {}) {
  ...
  const base = [
    ...
    { veh: VEHICULOS[1], pilotoId: 'avispita', poder: 'dirigido' },
  ];
  const arr = [];
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (carrera && r.cooldown <= 0) {
        ...
        if (r.pilotoId === 'jaguar' && dist < 26) {
          ...
          r.cooldown = 6.2;
        } else if (r.pilotoId === 'avispita' && dist < 34) {
          carrera.efectosJugador.push({ tipo: 'freeze', t: 1.0 });
          carrera.eventos.push({ tipo: 'poder', piloto: 'avispita', en: s.tiempo ?? 0, nombre: 'Tiro dirigido' });
          r.cooldown = 8.5;
        }
      }
      const st = {
