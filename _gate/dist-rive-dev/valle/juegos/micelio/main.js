...
import { RNG, clamp, lerp } from '../../lib3d/core/RNG.js';
...
const state = {
  ...
  mycoComplete: false,
  cell: null,
  // -- arrastre de raiz (Stage 2) --
  draggingRoot: false,
  rootDragStart: null,
  rootDragCurrent: null,
};
...
function setRootTarget(stage, target) {
  ...
  showToast(`Buscá ${target.label.toLowerCase()}. Arrastrá la raíz hacia el brillo.`);
}
...
function updateRootStage(stage, dt) {
  ...
  // La raíz SOLO crece cuando el jugador arrastra en pantalla.
  // Sin arrastre, la raíz se queda quieta (no hay crecimiento automático).
  if (state.draggingRoot && state.rootDragCurrent) {
    const dragDir = new THREE.Vector3(
      state.rootDragCurrent.x - (state.rootDragStart?.x ?? 0),
      -0.3,
      state.rootDragCurrent.y - (state.rootDragStart?.y ?? 0)
    );
    if (dragDir.lengthSq() > 1e-6) {
      dragDir.normalize();
      const speed = 3.0 * dt;
      p.current.addScaledVector(dragDir, speed);
      // Mantener la raíz dentro de los límites del suelo
      p.current.x = clamp(p.current.x, -5.5, 5.5);
      p.current.y = clamp(p.current.y, -2.5, 0.6);
      p.current.z = clamp(p.current.z, -5.5, 5.5);
    }
  }

  const destino = p.current;
  const bend = destino.clone().multiplyScalar(0.25);
  const base = new THREE.Vector3(0, 0.5, 0);
  const mid = new THREE.Vector3(bend.x * 0.3, 0.14 - bend.length() * 0.05, bend.z * 0.3);
  const tip = new THREE.Vector3(destino.x, destino.y, destino.z);
  ...
  // Absorción: si la punta está cerca de un target no tomado, lo absorbe
  for (const t of stage.targets) {
    if (!t.taken && tip.distanceTo(t.pos) < 0.55) {
      t.taken = true;
      stage.completedTargets += 1;
      stage.currentTarget = t;
      showToast(`${t.label} absorbido.`);
      const droplet = stage.droplets.find((d) => d.userData.targetId === t.id);
      if (droplet) droplet.visible = false;
      const label = stage.targetLabels.find((l) => l.element.textContent === t.label);
...
function createMycoStage() {
  ...
  for (const n of nodes) {
    ...
    const hit = new THREE.Mesh(
      new THREE.SphereGeometry(n.kind === 'fungus' ? 0.7 : 0.65, 10, 8),
  ...
  // Anillos indicadores alrededor de cada nodo seleccionable
  const ringTex = createGlowTexture('rgba(255,255,255,1)', 'rgba(255,255,255,0.85)', 128);
  for (const n of nodes) {
    const ring = new THREE.Sprite(new THREE.SpriteMaterial({
      map: ringTex,
      color: n.kind === 'fungus' ? 0xc9a86d : 0x6bb84b,
      transparent: true,
      opacity: 0.25,
      depthWrite: false,
      depthTest: false,
    }));
    ring.scale.setScalar(n.kind === 'fungus' ? 1.3 : 1.1);
  ...
  function addLink(aId, bId) {
    ...
    if (stageMyco.linksMade >= stageMyco.maxLinks) {
      showToast('Sin enlaces disponibles. La red ya está completa.');
      return null;
    }
    ...
    // Feedback según tipo de conexión
    if (kind === 'water' && (a.sick || b.sick)) {
      showToast(`${a.name} ↔ ${b.name} — agua directa a la planta enferma.`);
    } else if (kind === 'phosphorus') {
      showToast(`${a.name} ↔ ${b.name} — nutrientes fluyen.`);
    } else {
      showToast(`${a.name} ↔ ${b.name} — azúcar para el hongo.`);
    }

    // Sanación: la planta enferma recupera salud por CADA enlace correcto que la toca
    const touchesSick = (aId === 'planta-c' || bId === 'planta-c');
    if (touchesSick && stageMyco.sickNode.health < 1) {
      stageMyco.sickNode.health = Math.min(1, stageMyco.sickNode.health + 0.30);
    }

    // Victoria: planta sana con ≤ maxLinks
    if (stageMyco.sickNode.health >= 0.98 && !stageMyco.mycoComplete) {
      stageMyco.mycoComplete = true;
      stageMyco.done = true;
      showToast('La red sostuvo a la planta enferma.');
    }

    // Sin más enlaces disponibles
    if (stageMyco.linksMade >= stageMyco.maxLinks && !stageMyco.mycoComplete) {
      showToast('Sin enlaces. La planta no se recuperó.');
      stageMyco.done = true;
    }
...
function updateMycoStage(stage, dt) {
  ...
  for (const n of stage.nodes) {
    const pulse = 0.5 + Math.sin(now * 0.002 + n.pos.x * 3.1) * 0.06;
    ...
    // Glow ring: brilla más cuando el nodo está seleccionado
    if (n.glowRing) {
      const isSelected = stage.selected && stage.selected.id === n.id;
      const targetOp = isSelected ? 0.55 : 0.2;
      n.glowRing.material.opacity += (targetOp - n.glowRing.material.opacity) * 0.12;
      const s = n.kind === 'fungus' ? 1.3 : 1.1;
      const targetScale = isSelected ? s * 1.25 : s;
      n.glowRing.scale.lerp(new THREE.Vector3(targetScale, targetScale, targetScale), 0.1);
    }
  }
  for (const link of stage.links) {
    ...
    link.pulseA.scale.setScalar(0.2 + 0.08 * Math.sin((now + link.phase * 1000) * 0.01));
    link.pulseB.scale.setScalar(0.2 + 0.08 * Math.sin((now + link.phase * 1200) * 0.01));
  }
}
...
function updateHUD() {
  ...
  const total = state.stage === 0 ? s.parts.length : state.stage === 1 ? s.targets.length : state.stage === 2 ? s.maxLinks : (s.progress?.total || 0);
  let done = 0;
  if (state.stage === 0) done = s.learned ? s.learned.size : 0;
  if (state.stage === 1) done = s.completedTargets;
  if (state.stage === 2) done = s.linksMade;
  ...
  if (state.stage === 0) {
    ...
    const labels = [
      ['Arrastrá', 'para guiar la raíz'],
      ['Acercá', 'la punta a los brillos'],
      ['Absorbé', 'agua, fósforo y materia viva'],
    ];
    ...
    const sm = stages[2];
    const remaining = sm.maxLinks - sm.linksMade;
    const labels = [
      ['Enlaces', `${sm.linksMade} / ${sm.maxLinks} usados`],
      ['Toca', 'dos nodos para tejer'],
      ['Planta→Hongo', 'agua al enfermo'],
    ];
    if (remaining <= 1) labels.push(['Queda', `${remaining} enlace. ¡Pensá bien!`]);
    for (const [a, b] of labels) {
      const chip = document.createElement('div');
      chip.className = 'chip';
      chip.innerHTML = `<b>${a}</b> ${b}`;
...
renderer.domElement.addEventListener('pointerdown', (ev) => {
  ...
  setPointer(ev);
  // Stage 2: activar arrastre de raíz
  if (state.started && state.stage === 1) {
    state.draggingRoot = true;
    state.rootDragStart = { x: ev.clientX / innerWidth, y: ev.clientY / innerHeight };
    state.rootDragCurrent = { ...state.rootDragStart };
  }
});
renderer.domElement.addEventListener('pointermove', (ev) => {
  setPointer(ev);
  if (pointerDown && (Math.abs(ev.clientX - pointerDown.x) > 8 || Math.abs(ev.clientY - pointerDown.y) > 8)) pointerDown.moved = true;
  // Stage 2: actualizar posición de arrastre de raíz
  if (state.draggingRoot) {
    state.rootDragCurrent = { x: ev.clientX / innerWidth, y: ev.clientY / innerHeight };
  }
});
renderer.domElement.addEventListener('pointerup', (ev) => {
  ...
  // Stage 2: soltar raíz
  if (state.draggingRoot) {
    state.draggingRoot = false;
    state.rootDragStart = null;
    state.rootDragCurrent = null;
  }
...
function tick(now) {
  ...
  if (state.stage === 1) {
    elPrompt.textContent = 'Arrastrá para guiar la raíz. Llevála a los brillos de agua y nutrientes.';
  } else if (state.stage === 2) {
    const sm = stages[2];
    elPrompt.textContent = sm.done
      ? 'La red está completa.'
      : `Tejé la red. Tocá dos nodos para enlazarlos. Enlaces: ${sm.linksMade}/${sm.maxLinks}.`;
  } else if (state.stage === 3) {
...
function refreshStageState() {
  ...
  if (stage2.mycoComplete || stage2.linksMade >= stage2.maxLinks) stage2.done = true;
}
