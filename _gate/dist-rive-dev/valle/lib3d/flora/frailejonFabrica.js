...
const SPECIES = {
  grandiflora: {
    ...
    rosette: { layers: 3, leaves: 12, len: 1.34, width: 0.23, tilt: 0.98, silver: 0xc8d2bd },
    branching: false,
    flowers: true,
    tag: 'unicaule grande',
    pisoTermico: ['paramo'],
  ...
  argentea: {
    ...
    rosette: { layers: 3, leaves: 11, len: 1.24, width: 0.21, tilt: 1.02, silver: 0xd6dece },
    branching: false,
    flowers: false,
  ...
  killipii: {
    ...
    rosette: { layers: 3, leaves: 10, len: 1.14, width: 0.2, tilt: 0.96, silver: 0xc7cfb9 },
    branching: true,
    branchCount: 3,
    branchBase: 0.62,
    branchSpread: 0.42,
  ...
  lopezii: {
    ...
    rosette: { layers: 3, leaves: 11, len: 1.08, width: 0.19, tilt: 1.06, silver: 0xd0d9c9 },
    branching: false,
    flowers: false,
  ...
  pycnophylla: {
    ...
    rosette: { layers: 4, leaves: 14, len: 1.0, width: 0.18, tilt: 1.08, silver: 0xd6ddd2 },
    branching: false,
    flowers: true,
  ...
  uribei: {
    ...
    rosette: { layers: 3, leaves: 11, len: 1.1, width: 0.19, tilt: 1.02, silver: 0xcfd8c7 },
    branching: false,
    flowers: true,
    tag: 'Sumapaz lento',
    pisoTermico: ['paramo'],
...
function makeLeafGeo(len, width) {
  const geo = new THREE.CylinderGeometry(width * 0.04, width * 0.44, len, 5, 1, false);
...
function makeSkirtLeafGeo(len, width) {
  const geo = new THREE.CylinderGeometry(width * 0.06, width * 0.36, len, 5, 1, false);
  geo.rotateZ(-Math.PI / 2);
  geo.translate(len * 0.5, 0, 0);
  return geo;
}
...
function makeTrunkGeo(cfg) {
  const geo = new THREE.CylinderGeometry(cfg.r0, cfg.r1, cfg.h, 6, 1, false);
...
function addRosette(group, cfg, seed, silverOverride) {
  ...
  for (let layer = 0; layer < layers; layer++) {
    const count = cfg.rosette.leaves + layer * 3;
    const y = cfg.trunk.h + 0.04 + layer * 0.08;
    const lenMul = 1 - layer * 0.16;
...
function makeSimplified(cfg, seed) {
  ...
  const rand = rng(seed);
  const count = Math.max(8, cfg.rosette.leaves - 2);
  for (let i = 0; i < count; i++) {
