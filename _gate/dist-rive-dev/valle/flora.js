...
import { height, fbm, sstep, clamp, elevOf, CHANNEL_X, CHIFLON_X, SITE_X, SITE_Z, dirtAmount } from './terrain.js';
...
export function makeFlora(scene, { dirt = false } = {}) {
  ...
  for (const bk of [...bosqueBakes, oroBake, chachaBake]) {
  ...
  const pastPts = scatter(170, (x, y, z, s) =>
    elevOf(y) < 2460 && s < 0.2 && z < -180 && !inCorridor(x, z));
  ...
  const sietePts = scatter(110, (x, y, z, s) =>
    elevOf(y) > 2330 && elevOf(y) < 2700 && s < 0.6 && z > -420 && !inCorridor(x, z) && !nearCam(x, z));
  ...
  const palmPts = scatter(70, (x, y, z, s) =>
    elevOf(y) > 2250 && elevOf(y) < 2780 && s < 0.75 && z > -640 && !inCorridor(x, z) && !nearCam(x, z));
  ...
  const fernPts = scatter(160, (x, y, z, s) =>
    elevOf(y) > 2100 && elevOf(y) < 2620 && s > 0.22 && s < 1.0 && z < -80 && z > -680);
  ...
  const farPts = scatter(420, (x, y, z, s) =>
    z < -260 && z > -700 && elevOf(y) > 2150 && elevOf(y) < 2950 && s > 0.24 && s < 1.1 && !inCorridor(x, z));
  ...
  instTroncosRodal([...nearTroncos, ...pastTree.matrices, ...farTroncos], 'flora-bosque-troncos');
  ...
  const romMats = matricesMasa(romPts, { hBajo: 0.7, hAlto: 1.2, alturaNatural: romBake.alturaNatural, sink: 0.1 });
  group.add(
  ...
  const shrubMats = matricesMasa(shrubPts, { hBajo: 0.7, hAlto: 1.5, alturaNatural: shrubBake.alturaNatural, sink: 0.1 });
  group.add(
