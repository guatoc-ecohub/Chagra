...
estado = evaluar("""JSON.stringify((() => { const r = document.querySelector('.ca-atmosfera'); const op = (q) => { const e = document.querySelector(q); return e ? getComputedStyle(e).opacity : 'ausente'; };
  return { clima: r && r.dataset.clima, luz: r && r.dataset.luz, bancos: document.querySelectorAll('.ca-banco').length, jirones: document.querySelectorAll('.ca-jiron').length,
    capaNiebla: op('.ca-capa--niebla'), capaLluvia: op('.ca-capa--lluvia'), capaLuci: op('.ca-capa--luci'),
    capaEstrellas: op('.ca-capa--estrellas'), capaPolvo: op('.ca-capa--polvo'), rayos: op('.ca-rayos'),
    corriendo: document.getAnimations().filter((a) => a.playState === 'running').length, dpr: devicePixelRatio, w: innerWidth, h: innerHeight, ua: navigator.userAgent.slice(0, 80),
