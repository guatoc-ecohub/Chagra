...
const TAG = process.env.PROBE_TAG || 'auth7';
...
async function st() {
  try {
    return await withTimeout(page.evaluate(() => {
      ...
      return { login: q('#login-username'), onb: q('[data-testid="onb2-saltar-todo"]'), tile: q('[data-testid="tile-registrar-unificado"]'), reg: q('[data-testid="registro-unificado-manual"]'), regVoz: q('[data-testid="registro-voz"]'), regDone: q('[data-testid="registro-unificado-done"]'), head: txt.slice(0, 80).replace(/\n/g, ' ') };
    }), 5000, 'st');
...
if (dash) {
  ...
  for (let attempt = 0; attempt < 3 && !ok; attempt++) {
    ok = await withTimeout(page.evaluate(() => {
      const el = document.querySelector('[data-testid="tile-registrar-unificado"]');
      if (el) { el.click(); return true; }
      return false;
    }).catch(() => false), 6000, 'domclick');
  ...
  if (ok) {
    ...
    while (Date.now() < dl3) {
      await sleep(400);
      const s = await st();
      if (s.reg || s.regVoz || s.regDone) { got = s; break; }
    }
    await sleep(2500);
    const fin = await st();
    trans = { msToMarker: got ? mark() - tt : null, reached: !!got, regVoz: fin.regVoz, regManual: fin.reg, regDone: fin.regDone, head: fin.head };
  } else {
