...
#   2. vitest de los tests DIRECTOS de los archivos tocados           [DURO]
#      (co-localizados o en __tests__/; NO --changed, que arrastra el cierre
#       transitivo de importadores y con el la deuda roja del baseline).
#   3. tsc --noEmit  (proyecto)                                       [OFF por defecto]
#      -> baseline rojo (~4700 errores preexistentes) + 80s; OFF salvo RUN_TSC.
#         RUN_TSC=advisory informa; RUN_TSC=strict bloquea (solo si el area quedo limpia).
#   4. vite build                                                     [OPT-IN]
