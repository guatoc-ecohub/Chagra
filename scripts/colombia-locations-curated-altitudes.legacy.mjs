 ...
 *
 * #338: snapshot del antiguo `src/utils/colombiaLocations.js` (lista a mano de
 * 25 deptos / 117 municipios). YA NO es consumido por la app — el onboarding
 * usa el dataset DANE generado (`src/data/colombia-locations.dane.json`).
 *
 * Se conserva SOLO como fuente de altitudes (msnm) curadas a mano (IGAC/OSM),
 * que el DANE DIVIPOLA no publica. `gen-colombia-locations.mjs` lo importa en
 * tiempo de generacion y mergea esas altitudes por (departamento, municipio).
 * Vive en scripts/ (NO en src/) para no entrar al bundle del PWA.
 */
