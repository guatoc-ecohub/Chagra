...
export function matchByGenero(noncoPest, allPests) {
  ...
  for (const pest of allPests) {
    ...
    if (noncoGenus === pestGenus) {
      // Solo match si el NoncoPest es indeterminado ("sp" / "sp." como
      // ÚLTIMA palabra; "sp" suelto como substring falsaba p.ej. "spodoptera")
      const noncoBinomio = normalizeBinomio(noncoPest.nombre_cientifico || noncoPest.binomio);
      if (noncoBinomio && /\bsp\.?$/.test(noncoBinomio)) {
        matches.push({
          noncoPest: noncoPest.id,
          pest: pest.id,
          metodo: 'genero',
...
export function matchBySinonimos(noncoPest, allPests, pestSynonyms) {
  ...
  // Dedup por par (noncoPest, pest): un mismo par puede conectar por varios
  // sinónimos o por ambas direcciones; se reporta UNA sola vez.
  ...
  for (const pest of allPests) {
    ...
    for (const [synonym, canonical] of Object.entries(pestSynonyms)) {
      ...
      const llave = `${noncoPest.id}::${pest.id}`;
      if (vistos.has(llave)) break;
