...
async function login(page) {
  await page.evaluate(async (username) => {
    ...
    const result = await authMod.authenticateUser(username, 'diag5-pwd');
...
console.log('--- profile before click ---', await page.evaluate((u) => localStorage.getItem(`chagra:profile:v1:${u}`), USER));
await page.getByTestId('valle-marco-salir').click();
await page.waitForTimeout(1500);
console.log('--- profile after click ---', await page.evaluate((u) => localStorage.getItem(`chagra:profile:v1:${u}`), USER));
const marcoCount = await page.getByTestId('valle-marco-screen').count();
console.log('--- marco count after click ---', marcoCount);
