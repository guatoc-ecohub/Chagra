/**
 * recomposicion.test.js — el CANDADO de costuras del oso: recompone las
 * capas del kit con la MISMA matemática que pinta el navegador
 * (`mascaras()`, la sección pura de capas.js — no una copia de las
 ...
 *     midió 301 déficits (0,138%, máx 63,75/255) y 0 huecos en este kit:
 *     las bandas de las restas blandas `(1-m)` (0,5 over 0,5 = 0,75 —
 *     banda pálida alrededor de orejas y mandíbula).
 *
 ...
 * negativo quita la capa cabeza-render y exige déficit masivo; el positivo
 * compone la lámina contra sí misma y exige cero absoluto.
 */
...
const laminaFile = path.join(raiz, 'public', 'compai', 'laminas', 'oso.png');

/** Las capas que `hornearOso` PINTA (parche de párpado aparte — es overlay). */
const PINTADAS = ['mCuerpo', 'mCorona', 'mCabezaRender', 'mMandibula', 'mOrejaIzq', 'mOrejaDer'];
...
describe('recomposición del kit contra la lámina aprobada', () => {
  it('0 huecos (alfa compuesto == 0) y 0 déficits (alfaOriginal − alfaCompuesto > 0,5/255) sobre los ~218k píxeles visibles', () => {
    const m = medir(lamina, capas);
    expect(m.visible).toBeGreaterThan(200000);
  ...
  it('control NEGATIVO del medidor: sin la capa cabeza-render el déficit es masivo (el medidor distingue roto de sano)', () => {
