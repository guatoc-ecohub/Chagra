/**
 * recomposicion.test.js — el CANDADO de costuras del chivito punk: recompone
 * las capas con la MISMA matemática que pinta el navegador (`mascaras()`, la
 * sección pura de capas.js — no una copia de las fórmulas) y exige que el
 * compuesto devuelva la lámina aprobada EXACTA.
 *
 ...
 *     informe del lote (ops/INFORME-LOTE-LAMINA-ALCANCE-Y-CLAIM.md, rama
 *     audit/lote-lamina-alcance-claim), que midió 2.364 déficits (2,092%)
 *     con máximo 63,75/255 en este rig: las restas blandas (1-m) de
 *     mandíbula sobre cabeza y de cabeza sobre la mano del lápiz.
 *
 ...
 * negativo quita la capa cabeza y exige que el déficit sea masivo; el
 * positivo compone la lámina contra sí misma y exige cero absoluto.
 */
...
const laminaFile = path.join(raiz, 'public', 'compai', 'laminas', 'chivito-punk.png');
...
beforeAll(async () => {
  ...
  // Las 4 capas PINTADAS (las mismas que hornearChivito monta en el DOM),
  // horneadas con la MISMA fuente de verdad: mascaras() de capas.js.
  const defs = mascaras();
  capas = {};
  for (const nombre of ['mCuerpo', 'mCabezaRender', 'mMandibula', 'mManoLapiz']) {
...
describe('recomposición de las capas contra la lámina aprobada', () => {
  it('0 huecos (alfa compuesto == 0) y 0 déficits (alfaOriginal − alfaCompuesto > 0,5/255) sobre los ~113k píxeles visibles', () => {
