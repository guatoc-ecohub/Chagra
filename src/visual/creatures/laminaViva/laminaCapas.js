...
function parcheParpado(sd, W, H, ojo) {
  ...
  // fuente: centrada en el mismo cx, desplazada hacia arriba 1.5×r (fuera
  // del propio ojo, dentro de la cabeza en casi todas las poses medidas).
  // 1.5, no más: en cabezas chicas (luciérnaga, chivito) un desplazamiento
  // mayor cae fuera de la silueta (fondo transparente o el hueco entre
  // antena/crestas) — ver el peso por alfa de FUENTE más abajo, que es la
  // red de seguridad para cuando ni así alcanza.
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const j = (sy * W + sx) * 4;
      // alfa de FUENTE: un PNG transparente no tiene color de verdad fuera
      // de su silueta (RGB suele quedar en 0,0,0 — negro sin sentido). Sin
      // este peso, robar pixeles de un punto que cae en fondo transparente
      // pinta un parche NEGRO sólido sobre el ojo en vez de piel/pluma —
      // bug real, encontrado al verificar offline con sharp (luciérnaga y
      // chivito-punk: el offset caía en el hueco entre antena/cresta).
