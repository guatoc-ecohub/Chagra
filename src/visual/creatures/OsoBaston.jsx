...
/* EL OSO DEL BASTÓN — Tremarctos ornatus, EL CAMINANTE DE LOS ANDES.
   ...
   y polen acelerado. Su otro gesto es el `resopla` familiar de los oeres de la
 ...
 * erestiene la luna, panza de barriga amable (la referencia la tiene), y
...
export function OsoBaston({
  ...
  florece = false,
  /* RESOPLA (opt-in): el huff pesado familiar de los oeres de la casa — vaho
  ...
  const body = (
    <g className={`crt-body${vivo ? ' rh-boil' : ''}`} filter={`url(#${glow})`}>
      ...
      {/* Vaho del resoplido (el huff familiar de los oeres de la casa). */}
      {vaho}
    </g>
  );
