...
export function AutoDibujo({ as = 'path', stage, fade = false, className = undefined, ...rest }) {
  ...
  // createElement (y no <El …/>) porque `as` es un tag dinámico: con JSX, TS no
  // puede validar props contra la unión de todos los intrinsics.
  const drawProps = fade ? {} : { pathLength: 1 };
  return createElement(as, { className: cls, ...drawProps, ...rest });
}
