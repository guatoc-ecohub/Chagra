...
function Atmosfera({ anioRef, perfil }) {
  ...
  // El ref-escape-hatch de siempre (CamaraDirector/EscenaBoveda/Valle3D): la
  // escena de three no se reasigna directo — se guarda en un ref y se muta
  // por ahí (regla react-hooks/immutability).
  ...
  useLayoutEffect(() => {
    sceneRef.current.background = c.fondo;
    if (perfil.fog) sceneRef.current.fog = new THREE.Fog(CRUDO.niebla, CRUDO.nieblaCerca, CRUDO.nieblaLejos);
    return () => {
  ...
  useFrame(() => {
    ...
    if (sceneRef.current.fog && 'near' in sceneRef.current.fog) {
      const f = /** @type {THREE.Fog} */ (sceneRef.current.fog);
