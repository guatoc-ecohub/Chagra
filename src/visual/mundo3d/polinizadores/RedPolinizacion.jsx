...
export default function RedPolinizacion({ telar, tier = 'alto', reducedMotion = false, comoAbeja = false }) {
  ...
  // Ref-escape-hatch: el buffer (geo/pos/col) se muta vértice a vértice cada
  // frame — el useFrame de abajo lee la copia local vía ref, no el binding
  // devuelto directo por useMemo (regla react-hooks/immutability).
  const bufRef = useRef({ geo, pos, col, segsPorHilo });
  useEffect(() => { bufRef.current = { geo, pos, col, segsPorHilo }; }, [geo, pos, col, segsPorHilo]);
  ...
  useFrame((_, dtCrudo) => {
    ...
    const { geo, pos, col, segsPorHilo } = bufRef.current;
