...
function Diorama({ tier, reducedMotion, momento, comoAbeja, veneno, meliponarioAbierto, onDano }) {
  ...
  // evita el cascading render de react-hooks/set-state-in-effect — patrón
  // "Adjusting state when a prop changes" de los docs de React). Cada vez
  // que se apaga la amenaza, la finca se vuelve a poblar: los bichos vuelven
  // del monte y de las fincas vecinas. La recuperación es real y es lenta, y
  // depende de que quede monte de dónde volver.
  const [venenoPrevio, setVenenoPrevio] = useState(veneno);
  if (veneno !== venenoPrevio) {
    setVenenoPrevio(veneno);
