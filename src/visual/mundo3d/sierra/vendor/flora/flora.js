 ...
 *
 * Original: `~/demos/3d/lib3d/flora/flora.js` (valle). Se trae al bundle
...
// OCLUSIÓN HORNEADA POR ÁRBOL (Sylva s39, Token-Gremlin/realistic-forest MIT):
// atributo `aoHorneada` que nace en el bake de la copa (FollajeMasa) y se
// aplica al material sin SSAO. `?ao=0` deja los materiales sin parche (baseline).
import { aplicarOclusionHorneada, atributoOclusionTronco, configurarOclusionHorneada } from './lib3d/flora/FollajeMasa.js';
...
// ── EL DIAL DE DENSIDAD DEL VALLE (2026-09-02) ────────────────────────────
// `flora` es el 83 % de la geometría por frame (21,1 M de 25,5 M tris) y ~80 %
// del tiempo de frame (medido por ablación el 2026-09-02: quitarla vale
// +41,1 fps). La doctrina de la casa es explícita — «degradar DENSIDAD sí,
// degradar a low-poly NO» — pero la densidad NUNCA tuvo una perilla en el valle
// público: las mediciones de s10/s30 (+39,5 % y +55,2 %) son de BANCO, sobre
// `makeImpostorLOD`, que es otro camino de render y que el gate visual ya
// rechazó tres veces por «franja de bloques verdes».
//
// Esto NO cambia nada por defecto (1 = el valle de siempre, byte a byte: se
// siguen generando TODOS los puntos y se gastan las MISMAS llamadas a
// Math.random, así que el sorteo no se corre). Lo que agrega es la perilla que
// faltaba para poder MEDIR la palanca EN EL VALLE y ponerle un número al juicio
// de arte, en vez de extrapolar desde un banco:  `?floraDensidad=0.6`.
// El adelgazamiento es por HASH DETERMINISTA del índice, no por stride ni por
// re-sorteo: las plantas que quedan siguen EXACTAMENTE donde estaban (nada se
// mueve, nada cambia de sitio), sólo desaparecen algunas. Así el A/B visual
// compara el mismo valle con menos mata, que es la pregunta real.
const FLORA_DENSIDAD = (() => {
  const v = Number(new URLSearchParams(location.search).get('floraDensidad'));
  return Number.isFinite(v) && v > 0 && v <= 1 ? v : 1;
})();
function _hashDensidad01(i) {
...
function scatter(count, rule, avoidSite = true) {
  ...
  if (FLORA_DENSIDAD >= 1) return out;
  return out.filter((_, i) => _hashDensidad01(i) < FLORA_DENSIDAD);
}
...
function matricesMasa(pts, {
  hBajo = 1, hAlto = 2, alturaNatural = 1, sink = 0.3,
  ancho = 1, profundidad = ancho,
} = {}) {
  return pts.map((pt) => {
    ...
    // La altura conserva el rango ecológico del rodal; el ancho y el fondo son
    // la firma visible de especie. No se sustituye una especie por un color:
    // roble/nogal ocupan cielo lateral, encenillo/aliso dejan una aguja de copa.
    const s = new THREE.Vector3(k * ancho, k * (0.9 + Math.random() * 0.2), k * profundidad);
...
// (Exportados para sylva-s39-demo.html: el banco usa el código REAL del bosque.)
...
export function fusionarCopaMasa(bake) {
  ...
  const ao = new Float32Array(total).fill(1);   // Sylva s39: sin atributo = 1 (sin oclusión)
  let off = 0;
  for (const fuente of fuentes) {
    ...
    if (g.attributes.aoHorneada) ao.set(g.attributes.aoHorneada.array, off);
    off += n;
  }
  const out = new THREE.BufferGeometry();
  out.setAttribute('position', new THREE.BufferAttribute(pos, 3));
...
export function materialCopaMasaInstanciada(bake) {
  ...
  // Sylva s39: oclusión horneada (el atributo viaja en fusionarCopaMasa).
  if (configurarOclusionHorneada().activo) aplicarOclusionHorneada(THREE, mat);
  return mat;
}
...
export function makeFlora(scene, { dirt = false } = {}) {
  ...
  // Sylva s39: oclusión horneada por árbol. `?ao=0` = baseline (materiales sin
  // parche); `?aoF=` fuerza (0..1) y `?aoD=` cuánto atenúa también la luz directa.
  const qAO = new URLSearchParams(location.search);
  const aoActivo = configurarOclusionHorneada({
    activo: qAO.get('ao') !== '0',
    fuerza: qAO.has('aoF') ? Number(qAO.get('aoF')) : undefined,
    directo: qAO.has('aoD') ? Number(qAO.get('aoD')) : undefined,
  }).activo;
  ...
  // Sylva s41: cada especie conserva un perfil de corona, además del bake y
  // del tono. Las proporciones se aplican a la MISMA matriz de tronco y copa:
  // a media y larga distancia sigue quedando una silueta, no un tinte perdido
  // dentro de una masa verde. `?speciesVar=0` conserva un encenillo uniforme
  // para el A/B visual; el valor público por defecto es la mezcla nativa.
  const perfilesBosque = [
    { clave: 'roble_negro', nombre: 'roble', ancho: 1.26, profundidad: 1.16, tono: '#315b2b', claro: '#5d8140', oscuro: '#1b351b' },
    { clave: 'encenillo', nombre: 'encenillo', ancho: 0.84, profundidad: 0.88, tono: '#3d6d32', claro: '#6e9145', oscuro: '#203e20' },
    { clave: 'aliso_andino', nombre: 'aliso', ancho: 0.72, profundidad: 0.82, tono: '#477b3d', claro: '#7aa34f', oscuro: '#294d27' },
    { clave: 'nogal_andino', nombre: 'nogal', ancho: 1.14, profundidad: 1.06, tono: '#356b35', claro: '#65934b', oscuro: '#1d4124' },
  ];
  const speciesVariation = new URLSearchParams(location.search).get('speciesVar') !== '0';
  const bosqueBakes = perfilesBosque.map((perfil, i) => bakearArbolMasa(perfil.clave, {
    tono: perfil.tono, claro: perfil.claro, oscuro: perfil.oscuro, seed: floraSeed + 811 + i,
  }));
  ...
  const perfilesLejos = [perfilesBosque[1], perfilesBosque[2]];
  const bosqueLejosBakes = perfilesLejos.map((perfil, i) => bakearArbolMasa(perfil.clave, {
    tono: perfil.tono, claro: perfil.claro, oscuro: perfil.oscuro, seed: floraSeed + 821 + i, lejos: true,
  }));
  ...
  const arbolMasaScatter = (bake, pts, { hBajo = 8, hAlto = 14, sink = 0.3, perfil = perfilPorBake.get(bake) } = {}) => {
    const mats = matricesMasa(pts, {
      hBajo, hAlto, alturaNatural: bake.alturaNatural, sink,
      ancho: speciesVariation ? (perfil?.ancho ?? 1) : 1,
      profundidad: speciesVariation ? (perfil?.profundidad ?? 1) : 1,
    });
  ...
  // Sylva s39: contacto con el suelo horneado en el fuste (corteza de Sylva).
  atributoOclusionTronco(THREE, arbolTrunkGeo);
  if (aoActivo) aplicarOclusionHorneada(THREE, arbolTrunkMat);
  ...
  const repartirBosque = (pts) => {
    ...
    const oro = [];
    pts.forEach((pt, i) => {
      if (!speciesVariation) { formas[1].push(pt); return; }
  ...
  const matricesVerde = (pts, { hBajo, hAlto, alturaNatural, sink = 0.3, perfil = null }) => pts.map((pt) => {
    ...
    const ancho = speciesVariation ? (perfil?.ancho ?? 1) : 1;
    const profundidad = speciesVariation ? (perfil?.profundidad ?? 1) : 1;
    const esc = new THREE.Vector3(k * ancho, k * (0.9 + prandVerde() * 0.2), k * profundidad);
  ...
  const sembrarRodal = (bakes, pts, alturas) => {
    ...
    const baseUniforme = Math.max(0, bakes.findIndex((bake) => perfilPorBake.get(bake)?.nombre === 'encenillo'));
    pts.forEach((pt, i) => porForma[speciesVariation ? formaPara(pt, i) % bakes.length : baseUniforme].push(pt));
    porForma.forEach((sub, i) => {
      const mats = matricesVerde(sub, {
        ...alturas, alturaNatural: bakes[i].alturaNatural, perfil: perfilPorBake.get(bakes[i]),
      });
  ...
  // Contrato de inspección del gate: conteos y perfiles reales de ESTA carga.
  // No mide calidad visual; sólo evita llamar "variación" a un rodal que por
  // un guard de scatter haya terminado con una única especie.
  window.__floraSpecies = {
    activa: speciesVariation,
    semilla: floraSeed,
    perfiles: perfilesBosque.map((perfil, i) => ({
      nombre: perfil.nombre,
      ancho: perfil.ancho,
      profundidad: perfil.profundidad,
      instancias: matricesPorSilueta.get(bosqueBakes[i]).length,
    })),
    lejanas: perfilesLejos.map((perfil, i) => ({
      nombre: perfil.nombre,
      instancias: matricesPorSilueta.get(bosqueLejosBakes[i]).length,
    })),
  };
  scene.add(group);
