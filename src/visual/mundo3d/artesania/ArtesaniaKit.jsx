...
/* Memoiza una geometría y la libera al desmontar. Wrapper genérico a propósito
   (cada llamador arma su propia lista de deps): el lint estático de useMemo no
   puede verificar un array que no es literal en el sitio de la llamada. */
function useGeo(fabrica, deps) {
  // eslint-disable-next-line react-hooks/exhaustive-deps, react-hooks/use-memo
  const geo = useMemo(fabrica, deps);
...
function useMatTaller(nombre, perfil, extra, mapa) {
  const mat = useMemo(
    ...
    // JSON.stringify(extra/mapa) a propósito: son objetos de receta que cambian
    // por valor, no por referencia — comparar por referencia recrearía el
    // material en cada render.
    // eslint-disable-next-line react-hooks/exhaustive-deps, react-hooks/use-memo
