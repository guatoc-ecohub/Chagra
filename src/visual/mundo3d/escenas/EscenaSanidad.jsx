 ...
 * @returns {number|null}
 */
export function ratioSalud(saludFinca) {
 ...
 * @returns {number}
 */
export function matasDecaidas(ratio, total) {
...
function Diorama({ params, reducedMotion, fauna, estadoFinca }) {
  ...
  const matas = useMemo(() => {
    ...
    const nDecaidas = matasDecaidas(ratio, matasBase.length);
    const desde = matasBase.length - nDecaidas;
    return matasBase.map((m, i) => ({ ...m, decaida: i >= desde }));
  }, [estadoFinca?.saludFinca, matasBase]);
