...
describe('compaiRegistry.resolverCompai', () => {
  ...
  it('los cinco compañeros lámina-viva cruzan al portal con su cuerpo animado', () => {
    const esperados = {
      jaguar: JaguarLaminaViva,
      zariguya: ZariguyaLaminaViva,
      luciernaga: LuciernagaLaminaViva,
      'oso-baston': OsoBastonLaminaViva,
      'chivito-punk': ChivitoPunkLaminaViva,
    };

    for (const [tipo, LaminaViva] of Object.entries(esperados)) {
      const c = resolverCompai(tipo);
      expect(c.PortalComponent, `${tipo} sin lámina viva en el registro`).toBe(LaminaViva);
    }
  });
