...
describe('compaiRegistry.resolverCompai', () => {
  ...
  it('jaguar, zarigüeya y luciérnaga comparten la lámina viva en el portal', () => {
    const laminas = { jaguar: JaguarLaminaViva, zariguya: ZariguyaLaminaViva, luciernaga: LuciernagaLaminaViva };
    for (const tipo of Object.keys(laminas)) {
      expect(() => resolverCompai(tipo)).not.toThrow();
      const c = resolverCompai(tipo);
      expect(c.pendienteFable).toBe(false);
      expect(typeof c.EscenaComponent).toBe('function');
      expect(cuerpoPortalDe(c)).toBe(laminas[tipo]);
      expect(c.PortalComponent).toBe(laminas[tipo]);
    }
  });
