...
import {
  ...
  Skull, Wheat, Container, Camera, RefreshCw,
} from 'lucide-react';
import {
 ...
 * en punto, secar el grano y transformar; este módulo profundiza en GUARDAR a
 * mediano/largo plazo y CONSERVAR con seguridad. Cuatro pilares:
 ...
 * Fotos reales con licencia CC (autor + licencia + fuente SIEMPRE visibles, ver
 * /public/almacenamiento/creditos.json). CERO invención: las cifras salen de
 * almacenamientoCalculator.js (grounded al DR TRIPLE); lo que el DR marca "sin
 * respaldo suficiente" se muestra como grounded-pendiente honesto. La matemática
 * (proporción, geometría) es exacta.
 */

/* ── Créditos de foto (fuente única en el componente; espejo de
 *    /public/almacenamiento/creditos.json). Requisito de las licencias CC-BY:
 *    autor + licencia + enlace a la fuente, siempre visibles. ──────────────── */
const FOTOS = {
  'costales-maiz': { autor: 'Cut angles', licencia: 'CC BY-SA 4.0', fuenteUrl: 'https://commons.wikimedia.org/wiki/File:Sacks_of_dry_maize.jpg', alt: 'Costales de maíz seco apilados y listos para guardar en bodega.' },
  'troja-maiz': { autor: 'Jarekt', licencia: 'CC BY-SA 4.0', fuenteUrl: 'https://commons.wikimedia.org/wiki/File:Corn_crib_-_Floyd_County,_Virginia.jpg', alt: 'Troja de madera ventilada para guardar mazorca de maíz.' },
  'granero-tradicional': { autor: 'Wouter van Beek', licencia: 'CC BY-SA 3.0', fuenteUrl: 'https://commons.wikimedia.org/wiki/File:The_ritual_to_inaugurate_a_new_plaited_granary._Kapsiki.jpg', alt: 'Granero tradicional tejido y sellado con barro para guardar el grano de la familia.' },
...
/* ── Ilustración SVG propia: silo hermético que protege el grano. Theme-aware
 *    (usa --t-accent-rgb) y con el sol que "respira" (al-sun, se apaga con
...
function Hub({ onIr, onNavigate }) {
  const pilares = [
    { key: 'almacenar', icon: Warehouse, foto: 'troja-maiz', titulo: 'Almacenar', desc: 'Troja, silo hermético y métodos campesinos. Con la calculadora de lo que salva y de cuánto le cabe.', accent: 'amber' },
    { key: 'conservar', icon: FlaskConical, foto: 'secado-maiz', titulo: 'Conservar', desc: 'Secar, salar, ahumar, fermentar y enlatar — con el umbral de seguridad del botulismo.', accent: 'rose' },
    { key: 'plagas', icon: Bug, foto: 'plaga-gorgojo', titulo: 'Plagas de almacén', desc: 'El gorgojo y sus parientes, y cómo tumbarlos sin veneno, de lo barato a lo caro.', accent: 'lime' },
    { key: 'micotoxinas', icon: ShieldAlert, foto: 'moho-maiz', titulo: 'Micotoxinas', desc: 'Aflatoxina y fumonisina: el veneno invisible del grano guardado húmedo, y cuándo descartar.', accent: 'sky' },
  ];
  return (
    <div className="flex flex-col gap-4">
      {/* Gancho — foto real de la cosecha guardada + la cifra contundente del silo */}
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden">
        <Foto slug="costales-maiz" ratio="aspect-[16/9]" className="rounded-none border-0 border-b border-slate-800" objectPos="object-center" />
      ...
      {/* Las tres reglas de oro — legibles de un vistazo */}
      <ReglasDeOro />
          ...
          return (
            ...
            >
              <span className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 border border-slate-800">
                <img src={`/almacenamiento/${p.foto}.jpg`} alt="" aria-hidden="true" loading="lazy" className="w-full h-full object-cover" />
...
/* Las tres reglas de oro del buen almacén: seco, hermético, fresco. Legible de
 * un solo vistazo bajo el sol (íconos grandes, texto corto, contraste alto). */
function ReglasDeOro() {
  const reglas = [
    { icon: Sun, titulo: 'Seco', sub: 'Grano a ≤ 13 % antes de guardar. Húmedo = moho y micotoxina.', accent: 'amber' },
    { icon: ShieldCheck, titulo: 'Hermético', sub: 'Bien tapado: se acaba el aire y la plaga se muere sin veneno.', accent: 'emerald' },
    { icon: Snowflake, titulo: 'Fresco y limpio', sub: 'A la sombra, sin humedad, y la bodega limpia antes de llenar.', accent: 'sky' },
  ];
  return (
    <div className="grid grid-cols-3 gap-2">
      {reglas.map((r) => {
        const c = COLOR_MAP[r.accent];
        const Icon = r.icon;
        return (
          <div key={r.titulo} className={`rounded-xl border ${c.border} ${c.bg} p-2.5 text-center`}>
            <Icon size={22} className={`mx-auto ${c.text}`} aria-hidden="true" />
            <p className="mt-1.5 text-sm font-bold text-white leading-tight">{r.titulo}</p>
            <p className="mt-0.5 text-[11px] text-slate-300 leading-snug">{r.sub}</p>
          </div>
        );
      })}
    </div>
...
function PilarAlmacenar() {
  return (
    <div className="flex flex-col gap-5">
      {/* Firma del módulo: el silo hermético (SVG theme-aware, sol que respira) */}
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden">
        <div className="p-4 pb-2"><SiloIlustracion /></div>
        <p className="px-4 pb-4 text-sm text-slate-300 leading-relaxed">
          El buen almacén guarda el grano <span className="font-semibold text-slate-100">seco, cerrado y fresco</span>.
          Aquí calcule cuánto salva y cuánto le cabe, y elija la estructura que puede costear.
        </p>
      </section>
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {ESTRUCTURAS.map((e) => (
            <article key={e.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden">
              {e.id === 'troja' ? (
                <Foto slug="troja-maiz" ratio="aspect-[16/9]" className="rounded-none border-0 border-b border-slate-800" />
              ) : null}
              <div className="p-4">
      ...
      {/* Rotación: primero en entrar, primero en salir (PEPS) */}
      <RotacionPEPS />
    </div>
  );
}

/* Rotación PEPS — la regla que evita que lo viejo se dañe al fondo. Corta y
 * memorable, con ícono de reciclaje/rotación. */
function RotacionPEPS() {
  return (
    <section className="rounded-2xl border border-emerald-800/40 bg-emerald-950/20 p-4">
      <div className="flex items-center gap-2">
        <RefreshCw size={18} className="text-emerald-300 shrink-0" />
        <h3 className="text-[15px] font-bold text-emerald-200">Rotación: primero en entrar, primero en salir</h3>
      </div>
      <p className="text-sm text-emerald-100/90 mt-2 leading-relaxed">
        Marque cada bulto o frasco con la <span className="font-semibold">fecha de guardado</span> y consuma o venda primero
        lo más viejo. Así nada se queda añejo al fondo criando plaga u hongo.
      </p>
      <ul className="mt-3 flex flex-col gap-1.5 text-sm text-emerald-100/90">
        <li className="flex items-start gap-2"><span className="text-emerald-400 mt-0.5" aria-hidden="true">•</span><span className="leading-snug">Ponga adelante lo que entró primero; lo nuevo va al fondo.</span></li>
        <li className="flex items-start gap-2"><span className="text-emerald-400 mt-0.5" aria-hidden="true">•</span><span className="leading-snug">Revise cada mes: busque polvillo, calor, olor a moho o grano perforado.</span></li>
        <li className="flex items-start gap-2"><span className="text-emerald-400 mt-0.5" aria-hidden="true">•</span><span className="leading-snug">Nunca eche cosecha nueva encima de la vieja sin limpiar el recipiente.</span></li>
      </ul>
      <p className="mt-2 text-[11px] text-emerald-200/70">Buena práctica de manejo poscosecha (FAO) · confianza alta.</p>
    </section>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {CONSERVACION.map((m) => (
            <article key={m.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden">
              {m.id === 'secado' ? (
                <Foto slug="secado-maiz" ratio="aspect-[16/9]" className="rounded-none border-0 border-b border-slate-800" />
              ) : null}
              <div className="p-4">
...
function PilarPlagas() {
  return (
    <div className="flex flex-col gap-5">
      {/* Foto real del enemigo #1 + el porqué de la urgencia */}
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 overflow-hidden">
        <Foto slug="plaga-gorgojo" ratio="aspect-[16/9]" className="rounded-none border-0 border-b border-slate-800" />
        <p className="p-4 text-sm text-slate-300 leading-relaxed">
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {CONTROL_PLAGAS.map((c) => {
            ...
            return (
              <article key={c.id} className="rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden">
                {c.id === 'ceniza_diatomeas' ? (
                  <Foto slug="diatomeas" ratio="aspect-[16/9]" className="rounded-none border-0 border-b border-slate-800" />
                ) : null}
                <div className="p-3">
                  <div className="flex items-center gap-2 flex-wrap">
...
function PilarMicotoxinas({ onNavigate }) {
  return (
    <div className="flex flex-col gap-5">
      <section className="rounded-2xl border border-sky-800/40 bg-sky-950/20 overflow-hidden">
        <Foto slug="moho-maiz" ratio="aspect-[16/9]" className="rounded-none border-0 border-b border-sky-900/40" />
        <div className="p-4">
          <div className="flex items-center gap-2">
          ...
          <p className="text-sm text-sky-100/90 mt-2 leading-relaxed">
            ...
            Complementa el secado que ya calcula la <span className="font-semibold">Poscosecha</span>: aquí lo importante es no dejar que el moho crezca en el almacén.
          </p>
        </div>
      </section>
