...
import { getSuggestedCompanions, buildGuildPrompt, type CompanionSuggestion } from '../services/guildService';
import { SPECIES_DEFAULTS } from '../config/speciesDefaults';
import { CROP_TAXONOMY } from '../config/taxonomy';

/**
 * GuildSuggestions — Panel de compañeros sugeridos y antagonistas.
 */
interface GuildSuggestionsProps {
  speciesId: string | null | undefined;
  onSelectCompanion?: (speciesName: string) => void;
}
...
export const GuildSuggestions: React.FC<GuildSuggestionsProps> = ({ speciesId, onSelectCompanion }) => {
  ...
  const { companions, antagonists } = useMemo<{
    ...
    antagonists: CompanionSuggestion[];
  }>(() => {
  ...
  const handleAiQuery = async () => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        setAiSuggestions((parsed as AISuggestion[]).slice(0, 5));
      }
    } catch (err) {
