...
import { AlertTriangle, Clock, RefreshCw, Wifi, WifiOff, type LucideIcon } from 'lucide-react';
...
export default function PendingTasksWidget() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const pendingTasks = (await syncManager.fetchPendingTasksFromFarmOS()) as unknown as PendingTask[];
      console.log('📋 Tareas pendientes descargadas de FarmOS:', pendingTasks.length);
      setTasks(pendingTasks);
      const freshest = pendingTasks.reduce(
        (acc, t) => Math.max(acc, t.cachedAt || 0),
        0
      );
      ...
      try {
        const cachedTasks = (await syncManager.getPendingTasks()) as unknown as PendingTask[];
  ...
  const getSeverityConfig = (severity: string): SeverityConfig => {
    switch (severity) {
      case 'critical':
        return { color: 'bg-red-600', text: 'text-red-400', badge: 'CRÍTICA', icon: AlertTriangle };
      case 'high':
        return { color: 'bg-red-700', text: 'text-red-400', badge: 'ALTA', icon: AlertTriangle };
      case 'medium':
        return { color: 'bg-orange-600', text: 'text-orange-400', badge: 'MEDIA', icon: Clock };
      case 'low':
        return { color: 'bg-green-600', text: 'text-green-400', badge: 'BAJA', icon: Clock };
      default:
        return { color: 'bg-slate-600', text: 'text-slate-400', badge: 'N/D', icon: Clock };
    }
  };
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-base font-semibold text-slate-200 mb-2">{task.title}</p>
                </div>
