...
function PlantDossierCard({ dossier, altitudeM, onReloadCycles }) {
  const { label, slug, cycle, bioinsumos, relations, cycles } = dossier;
      ...
      <section>
        ...
        {cycle ? (
          <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-2">
            <PhenologyTimeline speciesSlug={slug} sowingDate={Date.now()} altitudeM={altitudeM} />
          </div>
        ) : (
...
export default function PlantaPorVozScreen({ onBack, onSave }) {
  ...
  // recargar una etapa desde CicloDetalle.
  const buildDossiers = useCallback(async (plants) => {
    ...
    return Promise.all(list.map((p) => buildPlantDossier(p).catch(() => null))).then((r) => r.filter(Boolean));
  }, []);
  ...
  // Listener para que el padre/tests sepan que se completó (paridad con VoiceCapture).
  useEffect(() => {
    if (phase === 'dossier' && dossiers.length === 0) {
      // Guardó pero sin slug resoluble: no hay dossier que mostrar → volver a captura.
      setPhase('capture');
    }
  }, [phase, dossiers.length]);
