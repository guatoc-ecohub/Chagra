...
async function collectEvents() {
  ...
  try {
    ...
    // DB may not be openable (e.g., tests)
  }
  ...
  if (error) {
    return (
      <div className="p-6 text-center">
        ...
        <button
          onClick={onRefresh}
          className="mt-3 px-3 py-1 bg-slate-800 text-slate-300 rounded text-xs hover:bg-slate-700"
        >
          Reintentar
        </button>
      </div>
    ...
    <div className="p-4 space-y-4" data-testid="pilot-telemetry-panel">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          title="Refrescar"
          aria-label="Refrescar telemetria"
        >
          <RefreshCw size={14} />
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
        {EVENT_TYPES.map((t) => (
          <div key={t} className="bg-slate-900/60 border border-slate-800 rounded-lg p-2 text-center">
            <p className="text-xl font-black text-emerald-400 tabular-nums" data-testid={`count-${t}`}>
              {counts[t] || 0}
            </p>
      ...
      {events.length === 0 && (
        <p className="text-slate-500 text-xs text-center py-4">Sin eventos registrados aun.</p>
      )}
...
export default function PilotTelemetryPanel() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      let service = null;
      try {
        service = await import('../services/pilotTelemetryService.js').catch(() => null);
      } catch (_) {
        service = null;
      }
