...
export function useAutosave(storageKey, initialState = {}) {
  ...
  const save = useCallback((partial) => {
    ...
    timer.current = setTimeout(() => {
      ...
      try { const p = { ...latest.current }; delete p.__submitted__; localStorage.setItem(key, JSON.stringify(p)); } catch (_) { /* ignore localStorage autosave errors */ }
    }, 2000);
  }, [key]);
  const clear = useCallback(() => {
    try { localStorage.removeItem(key); } catch (_) { /* ignore localStorage cleanup errors */ }
