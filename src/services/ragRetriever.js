 ...
 *
 * MERGE dev→main 2026-07-26: viene de `main` (#2579, fdb134a3). `dev` lo perdió
 * al reescribir flattenDoc; sin esto falla `ragClima.test.js > NO indexa
 * plumbing`, que AMBAS ramas traen. Se conserva.
 */
...
export function flattenDoc(doc, prefix = '', speciesSlug = null) {
  ...
  const passages = [];
  // MERGE dev→main 2026-07-26 — UNIÓN, no "gana un lado".
  // `main` (#2579) y `dev` arreglaron por separado el mismo bug (el umbral
  // `length > 20` botaba el 62% del saber). Se conserva la ESTRUCTURA de `dev`
  // (etiquetas legibles + filtro de basura `esTextoIndexable`) y las dos
  // GARANTÍAS de `main`, que `dev` perdió y que `ragClima.test.js` — test que
  // AMBAS ramas traen — verifica:
  //   1. el dato corto se indexa CON su clave ("calido" suelto no dice nada);
  //   2. el plumbing (id/slug/version) NO se indexa (diluye el IDF del BM25).
  const fieldLabel = (key) => formatKeyLabel(`${prefix}${key}`.replace(/\.$/, ''));
  const addPassage = (key, val) => {
    const path = `${prefix}${key}`;
    // Garantía 2 de `main`: plumbing fuera, en cualquier profundidad.
    if (esClaveRuido(path)) return;
    if (typeof val === 'string' && val.length > 20) {
      // Texto largo: entra entero y sin etiqueta, como en las dos ramas.
      passages.push({ key: path, text: val, species: slug });
    } else if (isContextualField(path, val) && (typeof val === 'string' || typeof val === 'number')) {
      passages.push({ key: path, text: buildContextualText(fieldLabel(key), val), species: slug });
    } else if (esTextoIndexable(val)) {
      // Garantía 1 de `main`: el dato corto viaja con su clave.
      passages.push({ key: path, text: `${fieldLabel(key)}: ${val.trim()}`, species: slug });
    } else if (Array.isArray(val)) {
      val.forEach((item, i) => {
        const itemPath = `${path}[${i}]`;
        if (typeof item === 'string' && item.length > 20) {
          passages.push({ key: itemPath, text: item, species: slug });
        } else if (isContextualField(itemPath, item) && (typeof item === 'string' || typeof item === 'number')) {
          ...
          passages.push({ key: itemPath, text: `${formatKeyLabel(itemPath)}: ${item.trim()}`, species: slug });
