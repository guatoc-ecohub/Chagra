...
describe('ttsService — preferencias de voz Kokoro (task #124)', () => {
  ...
  describe('getPreferredVoice', () => {
    ...
    it('ef_dora NO está en KOKORO_VOICES (control: retirada del catálogo, 2026-08-02)', () => {
      // El operador reportó que ef_dora suena "gringa" (prosodia anglo) y pidió
      // que NUNCA se oiga: se retira del catálogo (PR #2854) para que
      // toServableVoice la coercione a em_santa y el selector no la ofrezca.
      // Si alguien la reincorpora, este control falla avisando del cambio.
      const doraInVoices = KOKORO_VOICES.some(v => v.id === 'ef_dora');
      expect(doraInVoices).toBe(false);

      // Y si no está en el catálogo, tampoco debe persistirse ni servirse.
