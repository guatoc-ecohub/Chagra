 ...
 * con la CW: cubre el hueco cuando el sidecar no resuelve la entidad o cae.
 *
 * Anti-falso-positivo (CRÍTICO):
 ...
 *   - si la respuesta YA prohíbe el crudo de forma fuerte (`_responseAlreadyWarns`),
 *     no antepone el prefijo redundante (pero igual limpia ofertas sueltas).
 *   - idempotente por marcadores estables.
 *
 * Firma propia (solo el texto). Corre SIEMPRE (SAFETY).
 *
 * @param {string} responseText
 * @returns {{text:string, modified:boolean, reason:string|null}}
 */
export function guardToxicRawFoodConsumption(responseText) {
  if (typeof responseText !== 'string' || responseText.length === 0) {
    return { text: responseText ?? '', modified: false, reason: null };
  }
  const textNorm = _stripDiacritics(responseText);
  ...
  // Antepone el prefijo de seguridad si el cuerpo no advierte ya de forma fuerte.
  let prefixed = false;
  if (!_responseAlreadyWarns(_stripDiacritics(body)) && !body.includes(`${RAW_TOXIC_FOOD_PREFIX} ${food.names[0]}`)) {
    const prefix =
...
export function applyOutputGuards(
  ...
  const rawToxFoodRes = guardToxicRawFoodConsumption(text);
