...
import { Warehouse, MapPin, Eye, Package, Clock, ClipboardList, CheckCircle, WifiOff, Leaf, Mic, type LucideIcon } from 'lucide-react';
...
const InventoryDashboard = lazy(() =>
  import('./components/InventoryDashboard').then((m) => ({ default: m.InventoryDashboard }))
);
const FarmMap = lazy(() => import('./components/FarmMap'));
const WorkerDashboard = lazy(() =>
  import('./components/WorkerDashboard').then((m) => ({ default: m.WorkerDashboard }))
);
...
const DashboardView = React.memo(function DashboardView({
  ...
  const noGeoCount = useMemo(() => {
    ...
    return allAssets.filter((a) => {
      const geo = (a as { attributes?: Record<string, unknown> }).attributes?.intrinsic_geometry;
      return !geo || !(typeof geo === 'object' ? (geo as { value?: unknown }).value : geo);
    }).length;
  }, [plants, structures, lands]);

  const assetCounts = useMemo<AssetCount[]>(
    () => [
  ...
  return (
    <div className="h-[100dvh] w-full bg-slate-950 text-white flex flex-col overflow-hidden">
      <header className="p-4 border-b border-slate-800 shrink-0 flex justify-between items-center bg-slate-900/50 backdrop-blur-md sticky top-0 z-50 pt-[env(safe-area-inset-top)]">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-slate-400 hover:text-white px-4 py-3 min-h-[44px] bg-slate-800 rounded"
          >
            Salir
          </button>
        </div>
      ...
      <main className="flex-1 p-4 flex flex-col overflow-y-auto gap-4 bg-biopunk-pattern">
        <TelemetryAlerts lastFarmOsLog={lastLogMessage} onNavigate={onNavigate} />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {NAV_TILES.map((tile) => {
            ...
            return (
              ...
              >
                <TileIcon size={28} strokeWidth={2} className="mb-2" aria-hidden="true" />
...
export default function App() {
  const [currentView, setCurrentView] = useState<ScreenId>('loading');
  const [toast, setToast] = useState<ToastState | null>(null);
  const [lastLogMessage, setLastLogMessage] = useState('');

  useEffect(() => {
    const handler = (e: Event) => setLastLogMessage((e as CustomEvent<string>).detail);
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'mantenimiento':
        return (
          <MaintenanceScreen onBack={() => setCurrentView('dashboard')} onSave={showToast} />
        );
      case 'task_log':
      ...
      case 'javier':
        return (
          <ScreenShell
            title={`Campo — ${PRIMARY_WORKER_NAME}`}
            icon={Eye}
    ...
    <>
      ...
      {toast && (
        <div
          ...
          className={`fixed bottom-8 left-1/2 -translate-x-1/2 p-4 rounded-xl shadow-2xl flex items-center gap-4 z-50 w-11/12 max-w-sm border-2 pb-[max(2rem,env(safe-area-inset-bottom))] ${
            toast.isError
        ...
        >
          {toast.isError ? (
            <WifiOff size={32} className="shrink-0" aria-hidden="true" />
          ) : (
            <CheckCircle size={32} className="shrink-0" aria-hidden="true" />
          )}
