...
- Issue: https://github.com/guatoc-ecohub/Chagra/issues/430
- Title: [refine][catalog] trifolium_repens (trifolium_repens) — vp ≥200 chars + Tier A sources
...
**SPECIES EXISTENTE A REFINAR:** `trifolium_repens` — trifolium_repens ()

Estado actual en `catalog/chagra-catalog-seed-v3.1.json`:
- `valor_pedagogico` actual: 109 chars (objetivo ≥200)
- `source_ids` actuales: ['fao-ecocrop', 'restrepo-1996-abc-agricultura-organica']
...
- Notas propagación: Siembra al voleo; crea una alfombra densa que evita la erosión en los caminos de la finca chiguana.
- Estrato: bajo
- Compañeros: ['alnus_acuminata', 'zea_mays']
- Antagonistas: no registrado
- vp actual (para no perder lo bueno que ya hay): "Alfombra de vida. Fija nitrógeno para los frutales y praderas, permitiendo una nutrición constante del suelo."
...
- [ ] NO alterar otras species (solo `trifolium_repens`)

RESTRICCIONES:
- SOLO modificar `trifolium_repens` (no otras species)
- NO inventar datos sin fuente
- NO Wikipedia única
- NO platitudes

Prioridad P1. Batch refinamiento producción (batch 2 — top-6 de 10).
