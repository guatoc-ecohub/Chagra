//#region ../../home/kortux/Workspace/chagra/node_modules/ulid/dist/browser/index.js
...
function crockfordEncode(input) {
	...
	if (bitsRead > 0) output.unshift(buffer & 31);
...
function crockfordDecode(input) {
	...
	if (bitsRead >= 5 || buffer > 0) output.unshift(buffer & 255);
...
function decodeTime(id) {
	if (id.length !== 26) throw new ULIDError(ULIDErrorCode.DecodeTimeValueMalformed, "Malformed ULID");
	const time = id.substr(0, 10).toUpperCase().split("").reverse().reduce((carry, char, index) => {
	...
	if (time > 0xffffffffffff) throw new ULIDError(ULIDErrorCode.DecodeTimeValueMalformed, `Malformed ULID: timestamp too large: ${time}`);
	return time;
}
/**
...
*/
function isValid(id) {
	return typeof id === "string" && id.length === 26 && id.toUpperCase().split("").every((char) => ENCODING.indexOf(char) !== -1);
}
/**
...
function monotonicFactory(prng) {
	...
	return function _ulid(seedTime) {
		...
		return encodeTime(seed, 10) + newRandom;
	};
}
/**
* Generate a ULID
...
function ulidToUUID(ulid) {
	if (!ULID_REGEX.test(ulid)) throw new ULIDError(ULIDErrorCode.ULIDInvalid, `Invalid ULID: ${ulid}`);
...
function uuidToULID(uuid) {
	if (!UUID_REGEX.test(uuid)) throw new ULIDError(ULIDErrorCode.UUIDInvalid, `Invalid UUID: ${uuid}`);
	...
	return crockfordEncode(new Uint8Array(bytes.map((byte) => parseInt(byte, 16))));
}
//#endregion
