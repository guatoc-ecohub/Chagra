...
| **Guacamaya** (fila actualizada 2026-08-23 tras #2972 + paridad-angelita; el resto de la tabla es del audit 2026-08-21) | ✅ ambiente (bob/boil/parpadeo/cola) + **idle-cerebro real** (scheduler de `angelitaEstados.js`, `data-guaca-idle`, gate `idleCerebro`) | ✅ **vivo** — `hostALigero()` (#2972) reescribió los `:host(...)` a light DOM; visema real V1-V4 mueve `#picoBajo`, cabeza asiente, ala gesticula | ✅ `GuacamayaEntrada.jsx` (asoma→quieta→crece+aro→brillo, sin fase gafas a propósito), wireada en `src/mockups/GuacamayaViva.jsx` (ruta real `#/mockups/guacamaya-viva` en `App.jsx`) | ✅ `GuacamayaSalida.jsx` (aleta invita→desvanece), wireada en `GuacamayaViva.jsx` — **estrena el espejo que Angelita aún no tiene en dev** | ❌ `RevelacionMistica.jsx` no existe en dev (la salida hace su propio fade inline, no el barrido místico genérico) | ❌ sin escena 3D propia; cae 100% a Angelita en el mundo (`pendienteFable:true`) | ✅ señalar/sana/dispersar/amenaza/pacto **actúan de verdad** (post-`hostALigero`): ala al bosque, V de bandera, semillas, jaula | ✅ **10/10 con bypass rico** en `ChagraAgentAvatar.jsx` + `ESTADO_RIG_DE_ESTADO_AGENTE` (10 estados→7 poses del rig, mapeo documentado) + **gaze-follow** (pupilas siguen el puntero, mismos 4 estados de atención que Angelita) + **espejo `direccion`** + **`animated` fotograma digno** | ❌ sin accesorio equivalente (fase gafas omitida a propósito en la entrada) | ❌ sin `cuerpoDeClima`, sin prop `clima`, sin entrada en `ROPA_PERFIL_POR_BICHO` | ❌ no existe `guacamayaLamina/` en absoluto | ✅ `AVATAR_TYPES`, `ChagraAgentAvatarGuacamaya.jsx`→`GuacamayaCompai.jsx` + vitrina propia `GuacamayaViva.jsx` |
...
> **ACTUALIZACIÓN 2026-08-23 — guacamaya YA NO sufre esto.** PR #2972 introdujo
> `hostALigero()` (`arte-valle/nsRigValle.js`): reescribe los selectores
> `:host([data-estado="…"])` a forma light-DOM antes de inyectar el CSS, así las
> 47 reglas de estado del rig SÍ aplican. La rama de paridad con Angelita suma
> gaze-follow, espejo `direccion`, `animated`/`idleCerebro` y la vitrina
> `GuacamayaViva.jsx` (ver fila actualizada arriba). **Chivito-punk sigue con el
> problema tal como se describe abajo** (su migración es PR #2974, aparte).
