...
// ---------------------------------------------------------------------------
// "¿Los árboles se mueven?" — auditor de mundos 3D (Three.js), Node puro.
// Heurística de texto read-only. No es un parser de AST ni una verificación
// visual: solo busca señales de árbol y señales de movimiento, y pondera la
// cercanía entre ambas. Ver README.md para límites honestos.
// ---------------------------------------------------------------------------
...
const PATRONES_RENDERER = [
  { id: 'import_three', re: /\bfrom\s*['"]three['"]/g },
  { id: 'three_ns', re: /\bTHREE\s*\./g },
  { id: 'mesh', re: /\b[A-Z][A-Za-z0-9]*Mesh\b/g },
  { id: 'geometry', re: /\b[A-Z][A-Za-z0-9]*Geometry\b/g },
  { id: 'material', re: /\b[A-Z][A-Za-z0-9]*Material\b/g },
  { id: 'object3d', re: /\bObject3D\b/g },
];

const PATRONES_RENDERER_IMPORT = [{ id: 'import_three', re: /\bfrom\s*['"]three['"]/g }];

const PATRONES_BAKER = [
  { id: 'bake', re: /\bbake(?:d|r|s)?\b|\bbake(?=[A-Z_])/g },
  { id: 'precompute', re: /\bprecompute(?:d|s)?\b|\bprecompute(?=[A-Z_])/g },
  { id: 'serializ', re: /\bserializ(?:e|ed|es|ing)?\b|\bserializ(?=[A-Z_])/g },
];

// ---------------------------------------------------------------------------
// Máscara por línea: 'C' código, 'S' string, 'M' comentario.
// Los patrones de movimiento se evalúan solo sobre código+strings (nunca
// comentarios). Los de árbol se evalúan también sobre comentarios (evidencia
// débil, ver "arbolesSoloComentarios").
// ---------------------------------------------------------------------------
...
function tieneCoincidencias(lineas, patrones) {
  for (const linea of lineas) {
    if (coincidencias(linea, patrones).length > 0) return true;
  }
  return false;
}
...
export function analizarArchivo(rutaArchivo) {
  ...
  const cod = lineas.map((l, i) => filtrar(l, mascaras[i], CODIGO));
  ...
  const tieneRenderer = tieneCoincidencias(sig, PATRONES_RENDERER_IMPORT) || tieneCoincidencias(cod, PATRONES_RENDERER);
  const rutaLower = rutaArchivo.toLowerCase();
  const tieneBaker =
    tieneRenderer &&
    arboles.length > 0 &&
    (tieneCoincidencias(cod, PATRONES_BAKER) || /(?:^|[\/._-])(bake|baked|precompute|serializ)/.test(rutaLower));

  if (!tieneRenderer) {
    const primeraArbol = arboles[0] || null;
    const explicacionBase =
      'No usa Three.js de verdad (sin import de three, sin THREE., sin Mesh/Geometry/Material/Object3D); ' +
      'se excluye antes de juzgar movimiento.';
    const explicacion = primeraArbol
      ? `${explicacionBase} Aunque menciona árboles en ${primeraArbol.linea}, no dibuja escena 3D.`
      : explicacionBase;
    return {
      archivo: rutaArchivo,
      decision: 'NO_ES_RENDERER',
      confianza: 'alta',
      explicacion,
  ...
  if (tieneBaker) {
    const primerArbol = arboles[0] || null;
    const bakeDetalle = PATRONES_BAKER.flatMap((p) => {
      const halladas = [];
      for (let i = 0; i < cod.length; i++) {
        const encontrados = coincidencias(cod[i], [p]);
        for (const c of encontrados) halladas.push({ linea: i + 1, id: c.id, match: c.match });
      }
      return halladas;
    });
    const primerBake = bakeDetalle[0] || null;
    const detalleArbol = primerArbol ? ` Árbol en ${primerArbol.linea}.` : '';
    const detalleBake = primerBake ? ` Señal bake en ${primerBake.linea}.` : '';
    return {
      archivo: rutaArchivo,
      decision: 'BAKER',
      confianza: 'alta',
      explicacion:
        'Precomputa/serializa geometría de árbol y no toca el loop de render, así que se reporta aparte.' +
        detalleArbol +
        detalleBake,
      arboles: resumir(arboles),
      movimiento: [],
  ...
  // Shader de viento: vertexShader + "desplazamiento" (displacement/desplazamiento/pos.x) cerca.
  for (let i = 0; i < sig.length; i++) {
...
export function armarReporte(directorio) {
  ...
  const resumen = {
    ...
    NO_ES_RENDERER: 0,
    BAKER: 0,
    renderers_con_arboles_quietos: 0,
    renderers_total: 0,
    total: resultados.length,
  };
  for (const r of resultados) {
    if (Object.prototype.hasOwnProperty.call(resumen, r.decision)) resumen[r.decision]++;
    if (r.decision === 'CON_MOVIMIENTO' || r.decision === 'SIN_MOVIMIENTO') {
      resumen.renderers_total++;
    }
    if (r.decision === 'SIN_MOVIMIENTO') {
      resumen.renderers_con_arboles_quietos++;
    }
  }
...
function main() {
  ...
  if (directorios.length === 0) {
    process.stderr.write(
      'Uso: node auditor.mjs <directorio> [--json]\n' +
  ...
  process.stdout.write(
    `Titular: ${reporte.resumen.renderers_con_arboles_quietos} renderers con árboles quietos\n`
  );
  process.stdout.write(
    `Alcance: ${reporte.resumen.renderers_total} renderers, ${reporte.resumen.NO_ES_RENDERER} no-renderers, ` +
      `${reporte.resumen.BAKER} bakers, ${reporte.resumen.total} archivos\n`
  );
  process.stdout.write(
