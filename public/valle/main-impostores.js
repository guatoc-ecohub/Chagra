...
// ── Harness de medición (NO es el wiring final): sólo cambia de qué cliff se
//    importa. El orquestador hará esta misma línea en main.js a mano. ──
...
// El experimento impostor quedó archivado; el loop propio conserva el mismo
// reloj global y la misma entrada orgánica que main.js.
...
// La portada lo archivó el 08-12 (main.js:67-71) y esta entrada lo siguió
// publicando 3 días en 3d.guatoc.co/impostores-valle.html?mundo=bosque:
// una decisión cumplida en una entrada y falsa en la otra. Cerrado 2026-08-15.
...
const postfxMSAA = postfxOK && renderer.capabilities.isWebGL2 && renderer.capabilities.maxSamples >= 4;
window.__webgl = { webgl2: renderer.capabilities.isWebGL2, halfFloatColorExt, halfFloatTargetOK,
  maxSamples: renderer.capabilities.maxSamples, postfxOK, postfxMSAA };
...
// Espejo EXACTO de main.js (d989be1): esta entrada también sale a internet y
// el gate de linaje la marcaba ATRASADA por faltarle justo este bloque.
...
// ── construir el mundo ──
const { group: terrainG } = makeTerrain();
...
const floraPerf = makeFlora(scene);
...
// `renderer` NO es opcional: `makeParamo` se lo pasa a `wireParamoLecciones`,
// que sin él devuelve `null` en silencio y deja esta entrada publicada SIN las
// lecciones de suelo/micorrizas que la portada sí tiene. Las dos entradas se
// sirven al mismo dominio; la decisión tiene que ser la misma en las dos.
...
if (new URLSearchParams(location.search).get('juego') === 'ahorcado') {
  introDone = true;
  darControl();
...
bloomHalf();
// Hooks de perfilado: misma vara que main.js, incluyendo el brazo post=0.
...
const paseo = makePaseo({
  ...
  libre: () => introDone && libreOnb && !camMode && !leccionAgua && !paramoMundo
...
function adaptPR() {
  ...
  if (next !== prNow) {
    ...
    if (composer) composer.setPixelRatio(prNow); // el composer sigue al renderer
    if (composer) composer.setSize(innerWidth, innerHeight);
...
// En Mali, un draw instanciado que supera la capacidad de cualquiera de sus
// buffers puede perder el contexto WebGL y dejar la entrada en blanco. La
// guarda cubre InstancedMesh y geometrías instanciadas, incluyendo mundos que
// llegan por import() asíncrono.
...
function _avisarClamp(o, campo, from, to) {
  if (_clampVistos.has(o.uuid)) return;
...
renderer.setAnimationLoop(() => {
  ...
  // El páramo nativo (`?mundo=paramo`) devuelve un `update(t)` que aquí NADIE
  // llamaba: montado y congelado — sin vuelo entre estaciones, sin viento, sin
  // drift de nieblas y sin avance de las lecciones (`paramo.js:1329-1360`). La
  // portada cableó esto el 2026-08-07 (`main.js:1017`) y esta entrada se quedó
  // con el bug: la misma decisión, cumplida en una entrada y falsa en la otra —
  // el patrón exacto del campo de tierra del 08-10. Medido por el censo de
  // divergencias (`_gate/INFORME-ENTRADAS-DIVERGENCIA.md`, 2026-08-13).
  ...
  if (gradeo) gradeo.uniforms.uSemilla.value = (t % 1000);   // grano de película: semilla que avanza
  // Algunos mundos actualizan `count` durante su tick; guardar justo antes
  // del render cubre ese camino antes de emitir cualquier draw instanciado.
  clampInstancedDraws(scene);
  if (composer) composer.render();
  else renderer.render(scene, camera);
});

// El harness comparte el contrato del valle vivo: llegó a montar la escena y
// su loop de render; si no, el entrypoint publica __ARRANQUE_FALLO.
