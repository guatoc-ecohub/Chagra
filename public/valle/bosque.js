...
// ez-tree: el dosel del bosque (encenillo/roble) se hornea a la silueta ez-tree
// ramificada VIVA (Weinmannia / Quercus humboldtii, especies reales del catálogo)
// en vez del cono-invertido/domo-icosaedro parasol. Fusionado a UNA geometría por
// arquetipo → sigue siendo 1 InstancedMesh (sin subir draw calls).
...
// Devuelven UNA geometría fusionada, lista para instanciar. La luz baja del
// dosel al suelo: cada estrato trae su tinte horneado (dosel encendido, suelo
// casi sin sol) para que las tres capas se lean como capas.

// DOSEL · ENCENILLO (Weinmannia): CONO INVERTIDO — la copa se abre hacia arriba
// en una sola lámina sobre ramas delgadas y empinadas. Domina el techo.
function geomEncenillo(seed = 1) {
  const r = prng(seed), p = [];
  const H = 12 + r() * 3.5;
  p.push(pieza(new THREE.CylinderGeometry(0.28, 0.6, H, 6), C.encenillo, [0, H / 2, 0]));
  // ramas empinadas que abren hacia arriba
  for (let i = 0; i < 7; i++) {
    const a = (i / 7) * Math.PI * 2 + r();
    const lr = 2.6 + r() * 1.2;
    const br = new THREE.CylinderGeometry(0.06, 0.14, lr, 4);
    p.push(pieza(br, C.encenillo, [Math.cos(a) * lr * 0.32, H * 0.86, Math.sin(a) * lr * 0.32], [0, -a, -0.75]));
  }
  // la lámina de copa: cono INVERTIDO (base ancha arriba), verde VIVO de monte
  // (antes verde-gris "niebla": con la bruma bajada leía como copa muerta)
  p.push(pieza(new THREE.ConeGeometry(4.2, 3.4, 9), mezcla(C.niebla, C.brote, 0.45).getStyle(), [0, H + 0.4, 0], [Math.PI, 0, 0], [1, 1, 1]));
  p.push(pieza(new THREE.ConeGeometry(3.0, 2.2, 8), mezcla(C.niebla, C.monte, 0.4).getStyle(), [0, H - 0.6, 0], [Math.PI, 0, 0]));
  // TAPA soleada: el cono invertido es hueco y desde arriba se veía el interior
  // (disco NEGRO). La lámina que recibe el sol de frente cierra la copa en verde vivo.
  p.push(pieza(new THREE.CircleGeometry(4.15, 9), mezcla(C.brote, C.monte, 0.35).getStyle(), [0, H + 2.08, 0], [-Math.PI / 2, 0, 0]));
  return fusionar(p);
}

// DOSEL · ROBLE (Quercus humboldtii): copa domada, ancha y maciza sobre tronco
// gris fisurado. El árbol digno del piso; los emergentes de este bosque.
function geomRoble(seed = 2) {
  const r = prng(seed), p = [];
  const H = 10 + r() * 3;
  p.push(pieza(new THREE.CylinderGeometry(0.5, 0.9, H * 0.62, 7), C.roble, [0, H * 0.31, 0]));
  // horqueta de ramas gruesas
  for (let i = 0; i < 4; i++) {
    const a = (i / 4) * Math.PI * 2 + r() * 0.6;
    p.push(pieza(new THREE.CylinderGeometry(0.16, 0.34, 3.4, 5), C.roble, [Math.cos(a) * 1.1, H * 0.62, Math.sin(a) * 1.1], [0, -a, -0.5]));
  }
  // copa: bola domada de varios lóbulos (maciza), verde hoja coriácea
  const cop = mezcla(C.hojaCoriacea, C.monte, 0.35).getStyle();
  p.push(pieza(new THREE.IcosahedronGeometry(4.0, 1), cop, [0, H + 0.6, 0], null, [1.25, 0.82, 1.25]));
  for (let i = 0; i < 4; i++) {
    const a = (i / 4) * Math.PI * 2 + r();
    p.push(pieza(new THREE.IcosahedronGeometry(2.2 + r() * 0.7, 0), mezcla(C.hojaCoriacea, C.brote, 0.25).getStyle(),
      [Math.cos(a) * 2.6, H + 0.2 + r(), Math.sin(a) * 2.6], null, [1, 0.8, 1]));
  }
  return fusionar(p);
}

// DOSEL · PALMA DE CERA (Ceroxylon): columna pelada larguísima + penacho apical.
// Lo más alto — asoma por encima del dosel y no se confunde con nada.
function geomPalma(seed = 3) {
  const r = prng(seed), p = [];
  const H = 17 + r() * 4;
  p.push(pieza(new THREE.CylinderGeometry(0.22, 0.34, H, 7), C.yarumo, [0, H / 2, 0]));
  // anillos claros de la cera
  for (let k = 0; k < 8; k++) p.push(pieza(new THREE.TorusGeometry(0.26, 0.05, 4, 8), '#dfe3d6', [0, H * (0.2 + k * 0.09), 0], [Math.PI / 2, 0, 0]));
  // penacho: frondas radiales que caen
  for (let i = 0; i < 11; i++) {
    const a = (i / 11) * Math.PI * 2;
    const fr = new THREE.ConeGeometry(0.4, 5.2, 4);
    p.push(pieza(fr, mezcla(C.monte, C.brote, 0.4).getStyle(), [Math.cos(a) * 2.0, H + 0.4, Math.sin(a) * 2.0], [Math.sin(a) * 0.5, -a, Math.cos(a) * 0.5 - 0.4]));
  }
  return fusionar(p);
}

// SOTOBOSQUE · MANO DE OSO (Oreopanax): PARASOL de pocas hojas grandes
// trilobuladas — la garra del oso — sobre un tallo delgado.
function geomManoDeOso(seed = 5) {
  const r = prng(seed), p = [];
  const H = 3.4 + r() * 1.4;
  p.push(pieza(new THREE.CylinderGeometry(0.1, 0.18, H, 5), C.raicilla, [0, H / 2, 0]));
  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2 + r();
    // cada "mano": tres lóbulos (icosaedros aplanados) en abanico
    for (const off of [-0.5, 0, 0.5]) {
      const hoja = new THREE.IcosahedronGeometry(0.7, 0);
      p.push(pieza(hoja, mezcla(C.monte, C.hojaCoriacea, 0.4).getStyle(),
        [Math.cos(a + off * 0.28) * 1.1, H - 0.1, Math.sin(a + off * 0.28) * 1.1], null, [1, 0.18, 1]));
    }
  }
  return fusionar(p);
}
...
// SOTOBOSQUE · CHUSQUE (Chusquea): ABANICO de cañas finas desde una mata, sin
// copa — una masa rayada.
function geomChusque(seed = 7) {
  const r = prng(seed), p = [];
  for (let i = 0; i < 12; i++) {
    const a = rand(0, Math.PI * 2), lr = 3.2 + r() * 2.0, tilt = 0.12 + r() * 0.4;
    const cana = new THREE.CylinderGeometry(0.03, 0.06, lr, 4);
    p.push(pieza(cana, mezcla(C.brote, C.liquen, 0.3).getStyle(),
      [Math.cos(a) * 0.3, lr * 0.5, Math.sin(a) * 0.3], [Math.sin(a) * tilt, 0, -Math.cos(a) * tilt]));
    // hojitas lanceoladas arriba
    p.push(pieza(new THREE.ConeGeometry(0.5, 1.0, 4), C.brote, [Math.cos(a) * lr * tilt, lr * 0.9, Math.sin(a) * lr * tilt], null, [1, 0.3, 1]));
  }
  return fusionar(p);
}

// SOTOBOSQUE · ARBUSTO FLORECIDO (Tibouchina/Vallea): mata redonda multicaule
// salpicada de FLOR FUCSIA — la única mancha de color del sotobosque.
function geomArbustoFlor(seed = 8) {
  const r = prng(seed), p = [];
  const H = 2.2 + r() * 1.0;
  for (let i = 0; i < 4; i++) {
    const a = (i / 4) * Math.PI * 2 + r();
    p.push(pieza(new THREE.CylinderGeometry(0.05, 0.1, H, 4), C.raicilla, [Math.cos(a) * 0.3, H / 2, Math.sin(a) * 0.3], [0, 0, (r() - 0.5) * 0.4]));
  }
  p.push(pieza(new THREE.IcosahedronGeometry(1.5, 1), mezcla(C.monte, C.frioVivo, 0.4).getStyle(), [0, H, 0], null, [1.1, 0.9, 1.1]));
  // las flores fucsia
  const fl = prng(seed + 99);
  for (let i = 0; i < 18; i++) {
    const a = fl() * Math.PI * 2, ph = fl() * Math.PI, rr = 1.45;
    p.push(pieza(new THREE.IcosahedronGeometry(0.3, 0), C.fucsia,
      [Math.sin(ph) * Math.cos(a) * rr, H + Math.cos(ph) * rr * 0.9, Math.sin(ph) * Math.sin(a) * rr]));
  }
  return fusionar(p);
}
...
export function initBosque() {
  ...
  // cada arquetipo = una geometría fusionada + un InstancedMesh (pocos draw
  // calls por muchas matas). El claro central queda despejado para el Ent y la luz.
  const rr = prng(4242);
  ...
  // DOSEL — el techo de VERDAD (un bosque es un techo, no matas regadas):
  // denso fuera del claro para que desde adentro haya paredes de monte.
  // DOSEL ez-tree (silueta ramificada VIVA, no cono/domo parasol): encenillo
  // (Weinmannia, dominante altoandino) + roble negro (Quercus humboldtii, el
  // emergente digno). Horneados a UNA geometría vertex-color → 1 InstancedMesh
  // c/u, mismo costo de draw calls que los parasoles viejos. Tinte casado a la
  // paleta madre del bosque (C.encenillo tronco, verdes de monte en la copa).
  const geomEncenilloEz = bakearArquetipoFusion('encenillo', { alturaObjetivo: 13, tintHoja: mezcla(C.niebla, C.brote, 0.4).getStyle(), tintCorteza: C.encenillo });
  const geomRobleEz = bakearArquetipoFusion('roble_negro', { alturaObjetivo: 12, tintHoja: mezcla(C.hojaCoriacea, C.monte, 0.35).getStyle(), tintCorteza: C.roble });
  const encenillos = sembrar(geomEncenilloEz, 150, { rmin: 8, escala: [1.0, 1.45] });
  const robles = sembrar(geomRobleEz, 60, { rmin: 11, escala: [1.0, 1.4] });
  const palmas = sembrar(geomPalma(13), 12, { rmin: 28, escala: [0.8, 1.1] });  // los emergentes que rompen el techo
  // SOTOBOSQUE — a media altura, a la sombra
  sembrar(geomManoDeOso(15), 46, { rmin: 5, escala: [0.85, 1.25] });
  sembrar(geomHelechoArboreo(16), 44, { rmin: 5, escala: [0.85, 1.3] });
  sembrar(geomChusque(17), 52, { rmin: 4.5, escala: [0.7, 1.15] });
  sembrar(geomArbustoFlor(18), 40, { rmin: 5, escala: [0.8, 1.2], dentroClaro: false });
  sembrar(geomArbustoFlor(28), 12, { rmin: 6, escala: [0.85, 1.1], dentroClaro: true, borde: 0.2 });  // fucsia asomando al claro
  // sotobosque ASOMANDO al claro (que el estrato medio se lea en el cuadro hero)
  sembrar(geomHelechoArboreo(26), 10, { rmin: 6, escala: [0.9, 1.3], dentroClaro: true, borde: 0.18 });
  sembrar(geomManoDeOso(27), 8, { rmin: 7, escala: [0.9, 1.2], dentroClaro: true, borde: 0.18 });
  // SUELO — lo rastrero, CONCENTRADO alrededor del claro (donde la cámara vive):
  // regarlo por toda la ladera lo volvía polvo invisible y el claro leía potrero.
  sembrar(geomHelechoSuelo(10), 260, { rmin: 2.0, escala: [0.7, 1.3], dentroClaro: true, borde: 1.2 });
  ...
  {
    const datos = [
      ...encenillos.map((p) => ({ x: p.x, z: p.z, r: 4.4 })),
      ...robles.map((p) => ({ x: p.x, z: p.z, r: 5.2 })),
      ...palmas.map((p) => ({ x: p.x, z: p.z, r: 2.2 })),
    ];
  ...
  const nBrom = Math.min(110, (encenillos.length + robles.length) * 2);
  const brom = new THREE.InstancedMesh(bromeliaGeo, MAT_VEG, nBrom);
  {
    const m = new THREE.Matrix4(), q = new THREE.Quaternion(), s = new THREE.Vector3(), pos = new THREE.Vector3();
    const hosts = [...encenillos, ...robles]; let k = 0;
...
function construirEntRoble() {
  ...
  // COPA: masa domada de lóbulos (viva, se balancea) — verde hoja coriácea
  const copaG = new THREE.Group(); copaG.position.y = H * 0.5 + 3;
  const cop = [];
  cop.push(pieza(new THREE.IcosahedronGeometry(9, 2), mezcla(C.hojaCoriacea, C.monte, 0.35).getStyle(), [0, 0, 0], null, [1.35, 0.85, 1.35]));
  const rc = prng(88);
  for (let i = 0; i < 9; i++) {
    const a = rc() * Math.PI * 2, d = 4 + rc() * 4;
    cop.push(pieza(new THREE.IcosahedronGeometry(3 + rc() * 2, 1), mezcla(C.hojaCoriacea, C.brote, 0.3 + rc() * 0.2).getStyle(),
      [Math.cos(a) * d, (rc() - 0.4) * 5, Math.sin(a) * d], null, [1.1, 0.85, 1.1]));
  }
  // bromelias y flor de mortiño en la copa (epífitas del maestro)
  for (let i = 0; i < 10; i++) {
    const a = rc() * Math.PI * 2, d = 5 + rc() * 4;
    cop.push(pieza(new THREE.IcosahedronGeometry(0.4, 0), rc() > 0.5 ? C.fucsia : C.mortino,
      [Math.cos(a) * d, (rc() - 0.5) * 6, Math.sin(a) * d]));
  }
