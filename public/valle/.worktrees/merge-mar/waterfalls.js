...
import { CHORRERA_BEATS } from './juegos/chagra-kart/js/descenso-chorrera.js';
...
export function makeWaterfalls(scene, { seed = 20260811 } = {}) {
  ...
  // Envelope restaurado para el encuadre del valle: la implementación
  // histórica ya tenía tres saltos, pero su ancho se perdía a la distancia
  // del mirador. La referencia aérea pide una caída que ocupe la garganta,
  // con roca visible entre terrazas, no una línea blanca.
  const SEG = [
    { w: 20.0, bright: 1.34, impact: 0.9 },
    { w: 16.0, bright: 1.22, impact: 0.9 },
    { w: 24.0, bright: 1.30, impact: 1.0 },
  ];
  for (let s = 0; s < cuts.length - 1; s++) {
    ...
    const seg = buildFall(chorX, tA, tB, {
      base: segW, fan: last ? 1.6 : 1.1, splay: last ? 1.0 : 0.6, alphaK: 0.92,
  ...
  const pieX2 = chorX(BB), pieF = facePos(pieX2, BB);
  {
    ...
    const NR = 9;
    const rockGeo = crearGeometriaPiedraEsculpida(THREE, { seed: 41 });
    const rockMat = new THREE.MeshStandardMaterial({ color: 0x6f6a5e, vertexColors: true, roughness: 0.95, flatShading: true });
  ...
  // Landmark de la referencia dron: carpa azul pequeña en la meseta, a un
  // costado del nacimiento. Se posa sobre la misma cara que el agua, no en
  // coordenadas flotantes, y queda fuera del canal despejado.
  {
    const landmarkT = 0.73;
    const landmarkX = chorX(landmarkT) - 48;
    const landmarkFace = facePos(landmarkX, landmarkT);
    const carpa = new THREE.Mesh(
      new THREE.ConeGeometry(4.2, 3.8, 4),
      new THREE.MeshStandardMaterial({ color: 0x258cff, emissive: 0x06365f, emissiveIntensity: 0.45, roughness: 0.82, flatShading: true }),
    );
    carpa.name = 'chorrera-landmark-carpa-azul';
    carpa.position.set(landmarkX, landmarkFace.y + 3.0, landmarkFace.z + 16.0);
    carpa.rotation.y = Math.PI / 4;
    carpa.userData.perfSystem = 'agua';
    scene.add(carpa);
  }
...
export function makeKartChorrera(scene, { pista, seed = 20260811 } = {}) {
  ...
  const rockGeo = crearGeometriaPiedraEsculpida(THREE, { seed: seed + 18 });
  const rockMat = new THREE.MeshStandardMaterial({ color: 0x5c675d, vertexColors: true, roughness: 0.96, flatShading: true });
...
export function makeKartChorreraFase1(scene, { pista, seed = 20260811 } = {}) {
  ...
  const ledgeMat = new THREE.MeshStandardMaterial({ color: 0x3e5045, roughness: 0.98, flatShading: true });
  const ledgeGeo = crearGeometriaPiedraEsculpida(THREE, { seed: seed + 701 });
  const rockGeo = crearGeometriaPiedraEsculpida(THREE, { seed: seed + 702 });
  const rockMat = new THREE.MeshStandardMaterial({ color: 0x1c2925, vertexColors: true, roughness: 0.99, flatShading: true });
  mats.push(ledgeMat, rockMat);
  for (let i = 0; i < beats.length; i++) {
    const p = beatPoint(i);
    const ledge = new THREE.Mesh(ledgeGeo, ledgeMat);
    ledge.position.set(p.x, p.y + 0.8, p.z);
    ledge.scale.set(12, 0.55, 3.75);
  ...
  for (let i = 0; i < beats.length - 1; i++) {
    ...
    // Cada cortina nace en la altura física de la repisa y termina en la
    // siguiente. Así roca, poza y kart comparten el mismo desnivel.
    const ya = pa.y + (i === 0 ? 5.5 : 2.8);
    const yb = pb.y + 1.35;
  ...
  // inequívoca, a la izquierda del canal como en el frame de referencia.
  {
    ...
    const leftX = Math.sin(top.q.hdg), leftZ = -Math.cos(top.q.hdg);
    tent.position.set(top.x - leftX * 5.5, top.y + 3.0, top.z - leftZ * 5.5);
    ...
    flag.position.set(top.x + leftX * 4.8, top.y + 4.2, top.z + leftZ * 4.8);
  ...
  const bushMasa = crearCopaMasa(THREE, {
    esferas: [
      { c: [0, 1.0, 0], r: 1.0, esc: [1.25, 0.78, 1.15] },
      { c: [0.85, 0.72, 0.12], r: 0.72, esc: [1.0, 0.72, 0.9] },
      { c: [-0.72, 0.66, 0.18], r: 0.68, esc: [1.0, 0.7, 0.92] },
    ],
    seed: seed + 703,
    brillo: 0.08,
    cobertura: 1.35,
    ...
    maxCards: 260,
    modulacion: 0.36,
    nucleoOpts: { segs: [14, 10], rugosidad: 0.28, sombra: 0.5 },
    texOpts: { oscuro: '#1d3e28', medio: '#295536', claro: '#4e7d3b', hojas: 320, alargue: 2.2 },
    viento: { amplitud: 0.045, piso: 0.2, velocidad: 0.9 },
  }).grupo;
  for (let i = 0; i < 34; i++) {
    ...
    const bush = bushMasa.clone(true);
