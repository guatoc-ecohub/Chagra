/*
 * Implementación oceánica vendorizada para el mundo MAR.
 */
 ...
 *
 * [MAR-REAL · backport r160] El original (three r169) usaba la opción
 * `count` de WebGLRenderTarget para MRT; esa API llegó en r162. El kart
 * vendorea r160, donde el MRT es la clase aparte WebGLMultipleRenderTargets
 * y `rt.texture` ES el array. Aquí se elige la clase según `count` y se
 * expone `rt.textures` (alias del array) para que OceanFFT.js no cambie.
 */
export function makeRT(w, h, opts = {}) {
  ...
  if (count > 1) {
    const rt = new THREE.WebGLMultipleRenderTargets(W, H, count, common);
    // en r160 rt.texture ES el array de texturas del MRT
    rt.textures = rt.texture;
  ...
  const rt = new THREE.WebGLRenderTarget(W, H, common);
