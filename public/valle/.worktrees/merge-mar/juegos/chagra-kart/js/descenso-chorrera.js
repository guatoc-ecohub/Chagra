...
export function paredChorrera(f) {
  f = ((f % 1) + 1) % 1;
  if (f >= CHORRERA_TUNEL[0] && f <= CHORRERA_TUNEL[1]) return 34; // montaña
  if (f < 0.065) return 4 + 14 * suave((f - 0.03) / 0.035);   // meseta → borde
  if (f < 0.55) {
    ...
    return 26
      + 13 * cresta(f, 0.215, 0.06)
      + 14 * cresta(f, 0.50, 0.055);
  }
  if (f < 0.86) return 22 - 12 * suave((f - 0.55) / 0.12);    // valle abre
  return 14 + 20 * suave((f - 0.86) / 0.012);                  // boca del túnel
}
