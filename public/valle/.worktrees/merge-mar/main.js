...
import { makeClimaVivo } from './clima-vivo.js';
...
// ── BOSQUE ALTOANDINO ────────────────────────────────────────────────────────
// Mundo autónomo: árboles nativos en masa + matriz de páramo. Suprime el valle
// cuando termina de montar, como los demás mundos de pantalla completa.
...
let climaVivo = null;
{
  ...
  if (!_esNoche && ['lluvia', 'sol', 'niebla', 'arcoiris'].includes(_clima)) {
    ...
    climaVivo = makeClimaVivo({ scene, renderer, camera, atmos, terrainG, site });
    window.__climaVivo = climaVivo;
  }
}
...
// Un solo punto de verdad para el tamaño físico de toda la cadena de imagen.
// En una rotación, renderer, targets del composer, bloom y el texel del gradeo
// deben cambiar juntos. El tamaño del bloom recibe píxeles efectivos, no CSS.
function syncRenderSize(width = innerWidth, height = innerHeight) {
  ...
  if (bloom) {
    bloom.setSize(Math.max(1, Math.round(w * prNow / 2)),
                  Math.max(1, Math.round(h * prNow / 2)));
  }
  if (gradeo) gradeo.uniforms.uTexel.value.set(1 / w, 1 / h);
  window.__valleRenderSize = {
    css: [w, h],
    pixelRatio: prNow,
    drawing: [Math.round(w * prNow), Math.round(h * prNow)],
    composer: composer ? [Math.round(w * prNow), Math.round(h * prNow)] : null,
  };
}
syncRenderSize();
...
function adaptPR() {
  ...
  if (next !== prNow) {
    prNow = next;
    syncRenderSize();                            // renderer y post-proc cambian juntos
...
renderer.setAnimationLoop(() => {
  ...
  atmos.update(t);
  if (climaVivo) climaVivo.update(t);
  falls.update(t);
  site.update(t);
  portales.update(t);
  mundos.update(t);
