...
export function makeTerrain({ dirt = false } = {}) {
  ...
  const cCornice = new THREE.Color(0x8e8b80), cCorniceW = new THREE.Color(0xb3ae9d);
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    if (slope > 0.58) {
      ...
      // subpáramo: el bosque se achaparrra y dora hacia la cima
      tmp.copy(cForest).lerp(cForestL, band);
      tmp2.copy(cParamo1).lerp(cParamo2, band);
      tmp.lerp(tmp2, sstep(2820, 3020, elev) * 0.8 + 0.15);
      ...
      const rk = sstep(0.20, 0.38, slope);
      if (rk > 0) { tmp2.copy(cRockD).lerp(cRockL, band); tmp.lerp(tmp2, rk * 0.7); }
    }
    ...
    shader.fragmentShader = shader.fragmentShader
      ...
      .replace('#include <roughnessmap_fragment>',
        `#include <roughnessmap_fragment>
        {
          ...
          // roca SOLO donde de verdad empina: ~30°…~62°. Verde-dominante debajo.
          float scarp2 = smoothstep(0.22, 0.55, slope2);
          if (scarp2 > 0.001) {
            // roca gris-crema árida (coherente con las cornisas de file_158),
            // con moteado fbm para que no sea plana. NO gris-sucio oscuro.
            float mot = tFbm(vTerrWPos.xz*0.55 + vTerrWPos.y*0.15);
            vec3 rockCol = mix(vec3(0.34,0.31,0.27), vec3(0.55,0.52,0.46), mot);
            diffuseColor.rgb = mix(diffuseColor.rgb, rockCol, scarp2 * 0.48);
