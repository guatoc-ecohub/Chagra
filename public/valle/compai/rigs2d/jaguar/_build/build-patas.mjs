...
/* ════════════ 1) PATA DELANTERA LEJANA (blanca, vientre) ════════════ */
{
  ...
  for (let y = 200; y < H; y++) for (let x = 104; x < 212; x++) {
    // Borde medido de la garra blanca. El corte viejo dejaba un fragmento
    // x110..127 en cuerpo-inpaint, que luego quedaba fijo al mover la pata y
    // se leía como halo/mancha.
    const xLef = y < 350
      ? lerp(138, 124, ss(245, 280, y))
      : interp([[350, 139], [360, 116], [370, 110], [378, 110], [383, 115], [385, 121], [388, 164]], y);
    // En la garra la lámina ya trae el contorno: el feather hacia afuera
    // conserva ese borde sin dejarlo semitransparente al separar la capa.
    const fxL = y >= 350 ? ss(xLef - 4, xLef, x) : ss(xLef, xLef + 8, x);
  ...
  // La garra ya trae su contorno natural en la lámina. Después de y=382 el
  // trazo sintético cae fuera del alfa y forma el gancho oscuro que se veía
  // separado de la pata.
  for (let y = 292; y <= 382; y += 0.5) {
