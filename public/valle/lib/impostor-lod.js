...
void main() {
  ...
  vec4 mv = modelViewMatrix * vec4(centerW, 1.0);
  mv.xy += corner.xy * vec2(sx, sy);
  float hf = clamp(corner.y + 0.5, 0.0, 1.0);
  float sway = uWind * hf * sy * 0.05 * sin(uTime * 1.7 + centerW.x * 0.11 + centerW.z * 0.09);
  mv.x += sway;
...
void main() {
  ...
  float sway = uWind * hf * sy * 0.05 * sin(uTime * 1.7 + wp.x * 0.11 + wp.z * 0.09);
  wp.x += sway;
  wp.z += sway * 0.55;
...
export function makeImpostorLOD(opts = {}) {
  ...
  const nearU = { uTime: { value: 0 }, uWind: { value: wind } };
  ...
  nearMat.onBeforeCompile = (shader) => {
    shader.uniforms.uTime = nearU.uTime;
    ...
    shader.vertexShader = shader.vertexShader
      ...
      .replace('#include <begin_vertex>',
        ...
        '\tvec3 wp_ = ( instanceMatrix * vec4( transformed, 1.0 ) ).xyz;\n' +
        '\tfloat sw_ = uWind * hf_ * 0.07 * sin( uTime * 1.7 + wp_.x * 0.11 + wp_.z * 0.09 );\n' +
        '\ttransformed.x += sw_;\n' +
        '\ttransformed.z += sw_ * 0.55;');
  };
  ...
  function update(camera, dt) {
    ...
    _t += Math.min(dt, 0.05);
    baseU.uTime.value = _t;
    nearU.uTime.value = _t;
