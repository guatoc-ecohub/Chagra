...
const MUNDO_CHORRERA = (() => {
  const params = new URLSearchParams(location.search);
  return params.get('mundo') === 'chorrera' || params.get('vista') === 'chorrera';
})();
const pista = construirPista({ chorrera: MUNDO_CHORRERA });
...
function montarCoche() {
  ...
  const f0 = chorreraStart
    ? (Number.isFinite(gateF) ? Math.min(0.302, Math.max(0.205, gateF)) : 0.205)
    : 0.0;
...
function cuadroKart() {
  ...
  // física
  if (!(GATE_FREEZE && estado.corriendo)) fisica.step(dt, eAjustada);
  vigilarRescate(fisica);
