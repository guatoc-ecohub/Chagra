...
function montarCoche() {
  ...
  modelosProx.clear();
  const esDebug = location.search.includes('debug');
  const provocarRescate = () => {
  ...
  if (esDebug) {
    ...
    window.__provocarRescate = provocarRescate;
  }
  // El gate necesita el mismo disparador funcional sin abrir el resto de las
  // sondas de debug al juego normal. En producción, sin el parámetro, este
  // símbolo no existe.
  if (RESCATE_DEMO) window.__provocarRescate = provocarRescate;
}
...
const EXPOSICION_BASE = GATE_CHORRERA && !GATE_REAL ? 0.82 : (MUNDO_CHORRERA ? 1.0 : 1.05);
...
const RESCATE_DEMO = typeof location !== 'undefined'
  && new URLSearchParams(location.search).get('rescateDemo') === '1';
...
const AUTO_ROOM = typeof location !== 'undefined'
  ...
  : '';
let rescateDemoArmado = false;
function programarRescateDemo() {
  if (!RESCATE_DEMO || rescateDemoArmado) return;
  rescateDemoArmado = true;
  const esperarCarrera = () => {
    if (estado.fin) return;
    if (!estado.corriendo || typeof window.__provocarRescate !== 'function') {
      setTimeout(esperarCarrera, 100);
      return;
    }
    setTimeout(() => {
      if (!estado.fin) window.__provocarRescate();
    }, 2000);
  };
  esperarCarrera();
}
...
document.getElementById('startBtn').addEventListener('click', async () => {
  ...
  iniciarCountdown();
  programarRescateDemo();
});
document.getElementById('againBtn').addEventListener('click', () => location.reload());
if (AUTO_START || RESCATE_DEMO) {
...
function vigilarRescate(s) {
  ...
  entRescate.aparecer({ origen: s.rescateOrigen, destino: s.rescateDestino || s });
  sfx('rescate');
  cam.sacudir(0.72);
