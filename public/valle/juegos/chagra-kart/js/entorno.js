...
import { makeKartChorreraFase1 } from '../../../waterfalls.js';
...
export function crearEntorno(THREE, pista, cfg = {}) {
  ...
  const nivelesTerreno = controlSinLod
    ...
    : undefined;
  const terreno = crearTerrenoClipmap(THREE, pista, {
    material: terrenoMat,
  ...
  // La última vuelta baja por siete beats de La Chorrera. El adaptador comparte
  // los builders y shaders de waterfalls.js; no hay una copia de la cascada.
  const waterSeed = Number(new URLSearchParams(location.search).get('waterSeed')) || 20260811;
  const chorrera = makeKartChorreraFase1(escena, { pista, seed: waterSeed });
  ...
  const cinta = crearCintaPista(THREE, pista);
  ...
  const solDir = new THREE.Vector3(0.35, 0.18, -0.85).normalize();
  ...
  function puntoLateral(i, minOff, maxOff, salto) {
    ...
    for (let t = 0; t < 8; t++) {
      ...
      const info = pista.infoLocal(x, z);
      if (Math.abs(info.lat) > info.w + 3) return { x, z, y: pista.alturaMundo(x, z), zona: info.zona };
    }
    return null;
  }
  ...
  const frajN = Math.round(108 * follajeF);
  let puestosF = 0;
  ...
  const arbN = Math.round(92 * follajeF);
  let puestosA = 0;
  for (let i = 0; i < pista.n && puestosA < arbN; i += 4) {
    ...
    const tex = esNiebla ? texNiebla : texBosque;
    ...
    c.grupo.position.set(pt.x, pt.y, pt.z);
  ...
  const chusqN = Math.round(26 * follajeF);
  let puestosC = 0;
  ...
  const encN = Math.round(30 * follajeF);
  let puestosE = 0;
  for (let i = 0; i < pista.n && puestosE < encN; i += 4) {
    ...
    if (!pt) continue;
      ...
      en.position.set(pt.x, pt.y, pt.z);
  ...
  function actualizar(dt, s) {
    ...
    const meta = gateChorrera
      ...
      : (NIEBLA_ZONA[s.info?.zona ?? ZONA.PARAMO_ALTO] ?? NIEBLA_ZONA[ZONA.PARAMO_ALTO]);
    ...
    skyU.turbidity.value = gateChorrera ? 9.5 : 4 + t * 8;
    if (gateChorrera) {
