...
export function crearCintaPista(THREE, pista) {
  ...
  for (let i = 0; i < nAnillos && nE < maxEncaladas; i += 3) {
    ...
    // cal blanca con algo de mugre, cada piedra distinta
    cLima.setHex(0xe8e2d2).multiplyScalar(0.9 + hash01(x * 5.1, z * 4.3) * 0.14);
  ...
  for (let g = 0; g < nGrava; g++) {
    ...
    cGrava.setHex(claro > 0.45 ? 0xd8cdb0 : 0x8a7355).multiplyScalar(0.85 + claro * 0.3);
