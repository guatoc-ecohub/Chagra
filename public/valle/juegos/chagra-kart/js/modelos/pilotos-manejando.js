...
import { PILOTOS_CASA } from '../pilotos.js';
...
// Los compai pisan estos valores desde RASGOS_COMPAI. Dante, Oliver y Jaguar
// conservan sus rasgos propios más abajo.
...
export function crearPilotoManejando(THREE, opts = {}) {
  ...
  const tipo = PILOTOS_CASA.includes(tipoSolicitado) && Object.prototype.hasOwnProperty.call(PALETA, tipoSolicitado)
  ...
  const R = esComp ? RASGOS_COMPAI[tipo] : null;
  // opts.color es una PINTURA, no una especie. La especie se conserva en el
  // tipo permitido y el color solo puede teñir el material solicitado.
  if (opts.color != null) pal.cuerpo = opts.color;
  ...
  // pal.cuerpo manda en las patas; el TRONCO puede ir aparte en las variantes
  // que tienen una piel compuesta.
  ...
  const ojoMats = { esclera: mBlanco, iris: esJaguar ? mat(THREE, 0xe8a323, 0.55) : null };
  for (const lado of [-1, 1]) {
  ...
  if (esJaguar) {
    ...
    // tórax, abdomen, piezas bucales y antenas: todo de una, por compai
    const ctxEspecie = {
      THREEmat: (c, ro, me) => mat(THREE, c, ro, me),
      pal, pecho, pelvis, cabeza, partes, mBlanco, mNegro,
    };
    bocaBase = construirCompai(THREE, tipo, ctxEspecie).bocaBase;
  } else {
  ...
  if (tipo === 'dante') {
    ...
    // las antenas, orejas y crestas del compai ya las empujó el constructor
    // de especie a partes.orejas
  } else {
    for (const lado of [-1, 1]) {
...
export function aplicarPose(piloto, pose = {}) {
  ...
  // Algunas alas de compai permanecen plegadas hasta que el piloto acelera.
