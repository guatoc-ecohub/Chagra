...
export function montarSelector3D(opts = {}) {
  ...
  function obtenerVehiculo(id) {
    ...
    if (!e) {
      ...
      e = {
        ...
        pilotoNuevo: c.pilotoNuevo ?? null, pilotoTrazado: c.pilotoTrazado ?? null,
        pilotoViejo: c.piloto ?? null,
  ...
  function pilotoSuelto(id) {
    ...
    if (!p) {
      if (tienePilotoTrazado(id)) {
        const soporte = new THREE.Group();
        soporte.userData.chasis = soporte;
        p = montarPilotoTrazado(THREE, soporte, { tipo: id });
      } else {
      ...
      if (p.esTrazado) p.actualizar(POSE_VITRINA);
      else aplicarPose(p, POSE_VITRINA); // sin pose los miembros quedan degenerados
  ...
  function hornear(cupo) {
    ...
    if (cupo.tipo === 'veh') {
      ...
      for (const g of [e.pilotoNuevo?.grupo, e.pilotoTrazado?.grupo, e.pilotoViejo?.grupo]) {
    ...
    // Los SVG llegan por Blob de forma asíncrona. No hornear un impostor vacío:
    // se devuelve a la cola y el siguiente cuadro lo captura cuando la textura
    // ya está lista.
    if (obj.userData?.trazado && !obj.userData.trazadoListo) {
      // Mantenerlo al frente evita que los diez retratos siguientes consuman
      // toda la cola mientras jaguar/oso esperan la decodificación del SVG.
      cola.unshift(cupo);
      return;
    }
  ...
  function cuadro() {
    ...
    const pil = modeloVivo?.pilotoNuevo ?? modeloVivo?.pilotoTrazado;
    if (pil) {
