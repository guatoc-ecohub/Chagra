...
scene.background = new THREE.Color(0xb8c9b6);
scene.fog = new THREE.FogExp2(0xc2d1c0, 0.006);

const camera = new THREE.PerspectiveCamera(50, innerWidth / innerHeight, 0.1, 800);
camera.position.set(-22, 18, 36);
...
const hemi = new THREE.HemisphereLight(0xd8e8d0, 0x3d4a2e, 1.3);
scene.add(hemi);
const sun = new THREE.DirectionalLight(0xfff4d0, 1.5);
sun.position.set(-0.3, 0.92, 0.25);
scene.add(sun);
const fill = new THREE.DirectionalLight(0x9ec4ab, 0.38);
...
function terrainColor(x, z, y) {
  ...
  const moss = new THREE.Color('#4a5e32');
  const leaf = new THREE.Color('#7a9a50');
  const lichen = new THREE.Color('#b5c890');
  const soil = new THREE.Color('#3a3d28');
  const rock = new THREE.Color('#5e6850');
  const wetTone = new THREE.Color('#547044');
  ...
  c.multiplyScalar(0.82 + grain * 0.18);
  return c;
}
function createGroundTexture() {
  ...
  for (let i = 0; i < 900; i++) {
    ...
    const t = Math.random();
    ctx.fillStyle = t < 0.3 ? 'rgba(30,42,22,0.15)' : t < 0.7 ? 'rgba(160,180,110,0.1)' : 'rgba(80,70,45,0.12)';
    ctx.beginPath();
  ...
  for (let i = 0; i < 1200; i++) {
    ...
    ctx.fillStyle = Math.random() < 0.5 ? 'rgba(20,35,15,0.4)' : 'rgba(210,200,160,0.12)';
    ctx.beginPath();
  ...
  for (let i = 0; i < 220; i++) {
    ...
    ctx.strokeStyle = Math.random() < 0.6 ? 'rgba(170,195,120,0.2)' : 'rgba(60,78,42,0.18)';
...
createRidgeBand({ z: -95, scale: 1.5, color: '#5a6e4a', peaks: 5, amp: 9, depth: 28 });
createRidgeBand({ z: -70, scale: 1.15, color: '#749060', peaks: 7, amp: 7.2, depth: 22 });
createRidgeBand({ z: -44, scale: 0.92, color: '#8aaa72', peaks: 8, amp: 5.4, depth: 18 });
createRidgeBand({ z: 24, scale: 0.82, color: '#809a68', peaks: 6, amp: 5, depth: 18 });
createRidgeBand({ z: 76, scale: 1.15, color: '#6a8050', peaks: 5, amp: 6.8, depth: 26 });
...
function makeRock(seed, scale = 1) {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: '#5a6a4c', roughness: 1, metalness: 0 });
  const core = new THREE.Mesh(new THREE.SphereGeometry(1, 18, 14), mat);
  core.scale.set(1.1 + rand() * 0.45, 0.75 + rand() * 0.3, 0.95 + rand() * 0.4);
  g.add(core);
  const cap = new THREE.Mesh(new THREE.SphereGeometry(0.55, 16, 12), new THREE.MeshStandardMaterial({ color: '#7a8a68', roughness: 1 }));
...
function placeFrailejon(key, x, z, scale, seed) {
  ...
  tagInteractive(lod, item);
  item.root = lod;
  interactives.push(item);
  interactiveRoots.push(lod);
  world.add(lod);
...
function placeBear(x, z, seed) {
  ...
  tagInteractive(bear, item);
  item.root = bear;
  interactives.push(item);
  interactiveRoots.push(bear);
  world.add(bear);
  return bear;
}
...
function placeVenado(x, z, seed) {
  ...
  tagInteractive(deer, item);
  item.root = deer;
  interactives.push(item);
  interactiveRoots.push(deer);
  world.add(deer);
  return deer;
}
...
function placeCondor(x, z, seed) {
  ...
  tagInteractive(condor, item);
  item.root = condor;
  interactives.push(item);
  interactiveRoots.push(condor);
  world.add(condor);
  return condor;
}
...
const pathLine = new THREE.Line(
  ...
  new THREE.LineBasicMaterial({ color: 0xe8dcc0, transparent: true, opacity: 0.42 })
);
world.add(pathLine);
...
const infoState = { pinned: null, current: null };

canvas.addEventListener('pointerup', () => {
  if (currentPick) {
    infoState.pinned = currentPick;
    infoEl.querySelector('h2').textContent = currentPick.text.title || currentPick.name;
    infoEl.querySelector('p').textContent = currentPick.text.body || '';
    infoEl.querySelector('.tiny').textContent = `${currentPick.name} · ${currentPick.scientific}`;
...
function updateAiming() {
  ...
  for (const hit of hits) {
    let o = hit.object;
    while (o) {
      if (o.userData?.item) { picked = o.userData.item; break; }
      o = o.parent;
    }
    if (picked) break;
  }
  ...
  if (!infoState.pinned && picked) {
    infoEl.querySelector('h2').textContent = picked.text.title || picked.name;
    infoEl.querySelector('p').textContent = picked.text.body || '';
    infoEl.querySelector('.tiny').textContent = `${picked.name} · ${picked.scientific}`;
  }
  if (!picked && !infoState.pinned) {
    infoEl.querySelector('h2').textContent = 'Acérquese a un frailejón o a un animal';
    infoEl.querySelector('p').textContent = 'Cada punto del paisaje cuenta algo real. La cámara orbital le permite explorar el páramo desde cualquier ángulo.';
    infoEl.querySelector('.tiny').textContent = 'Mirada Humboldt';
  }
}
