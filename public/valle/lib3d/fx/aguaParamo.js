...
//
// La lectura de flujo longitudinal, las vetas de brillo y el contacto de
// orilla toman inspiración técnica de secret_river de SamG-Coder/threepp
// (MIT). Están reimplementados aquí como ruido procedural propio: no hay
// texturas externas, reflector, refracción ni render-target.
...
// Geometría canónica de piedra emergente. La comparten los adaptadores que
// necesitan pedrero fuera de crearQuebrada: mismo icosaedro abollado, misma
// lectura de luz horneada y normales recalculadas; no usar sólidos regulares
// (DodecahedronGeometry/BoxGeometry) para representar roca del páramo.
export function crearGeometriaPiedraEsculpida(THREE, { seed = 1 } = {}) {
  const geo = new THREE.IcosahedronGeometry(1, 1);
  const pp = geo.attributes.position;
  const cc = new Float32Array(pp.count * 3);
  for (let i = 0; i < pp.count; i++) {
    const vx = pp.getX(i), vy = pp.getY(i), vz = pp.getZ(i);
    const r = 1 + (vnoiseA(vx * 2.1 + 5 + seed * 0.17, vz * 2.1 - 3 - seed * 0.11) - 0.5) * 0.34;
 ...
 *   flujoDireccional {boolean} flujo longitudinal GLSL (def true)
 *   vetas            {boolean} vetas/glints GLSL (def true)
 *   espumaOrilla     {boolean} contacto procedural de orilla GLSL (def true)
 * @returns {{ grupo, tick, contarTriangulos, curva, enCurso, profundidadEn,
 *             efectos }} `efectos.{flujoDireccional,vetas,espumaOrilla}` son
 *   uniforms float (0 apaga, 1 enciende) para ajustes aditivos en caliente.
...
export function crearQuebrada(THREE, opts = {}) {
  ...
  grupo.name = 'quebrada';
  // Banderas separadas: los tres aportes GLSL son aditivos y se pueden apagar
  // sin reinstanciar el material (útil para A/B en Mali y en los gates).
  const uniforms = {
    uTiempo: { value: 1.7 },
    uFlujoDireccional: { value: opts.flujoDireccional === false ? 0 : 1 },
    uVetas: { value: opts.vetas === false ? 0 : 1 },
    uEspumaOrilla: { value: opts.espumaOrilla === false ? 0 : 1 },
  };
  ...
  const NW = 7;
  {
    ...
    const mat = new THREE.ShaderMaterial({
      uniforms: THREE.UniformsUtils.merge([
        THREE.UniformsLib.fog,
        {
          ...
          uFlujoDireccional: uniforms.uFlujoDireccional,
          uVetas: uniforms.uVetas,
          uEspumaOrilla: uniforms.uEspumaOrilla,
        },
      ]),
      vertexShader: /* glsl */ `
      ...
      fragmentShader: /* glsl */ `
        ...
        uniform float uFlujoDireccional, uVetas, uEspumaOrilla;
        varying vec2 vUv;
        varying vec3 vInfo;
        varying vec3 vW;
        varying vec3 vNg;
        ...
        ${NOISE_GLSL}
        // Base isotrópica de la lámina: mantiene el rizo fino ya aprobado.
        ...
        // Campo anisotrópico reimplementado: X es ancho del cauce y Y son
        // metros aguas abajo. Al variar más a lo ancho que a lo largo, sus
        // crestas se estiran con la corriente y la advección baja el cauce.
        float campoFlujo(vec2 q, float t) {
          float meandro = (noiseG(vec2(q.y * 0.13 - t * 0.18, 5.7)) - 0.5) * 0.36;
          float a = noiseG(vec2((q.x + meandro) * 2.5, q.y * 0.34 - t * 1.18));
          float b = noiseG(vec2((q.x - meandro * 0.6) * 6.4 + 8.3, q.y * 0.78 - t * 2.35));
          return a * 0.64 + b * 0.36;
        }
        // Hilos largos que sólo se ven cuando la luz especular los alcanza.
        // No es una textura: es la misma coordenada de flujo, muy comprimida
        // longitudinalmente y advectada a una velocidad distinta.
        float vetasFlujo(vec2 q, float t) {
          float deriva = (noiseG(vec2(q.y * 0.10 - t * 0.16, 12.4)) - 0.5) * 0.52;
          return noiseG(vec2((q.x + deriva) * 11.0 + 19.1, q.y * 0.16 - t * 0.92));
        }
        void main() {
          float rap = vInfo.y;
          ...
          float t = uTiempo * vel;
          float hBase = campo(q, t);
          float h = mix(hBase, campoFlujo(q, t), uFlujoDireccional);
          // normal por diferencias finitas: EL movimiento que vende el agua
          float e = 0.05;
          float hx = mix(campo(q + vec2(e, 0.0), t), campoFlujo(q + vec2(e, 0.0), t), uFlujoDireccional);
          float hy = mix(campo(q + vec2(0.0, e), t), campoFlujo(q + vec2(0.0, e), t), uFlujoDireccional);
          float gx = hx - h;
          float gy = hy - h;
          float k = 2.1 + rap * 2.6;
          ...
          float veta = smoothstep(0.68, 0.88, vetasFlujo(q, t));
          float brilloVeta = veta * pow(max(dot(N, Hv), 0.0), 42.0) * uVetas;
          ...
          float m = vInfo.z;
          // Contacto de orilla sin textura ni depth: uv.x es la distancia
          // lateral conocida por la cinta. La trama advectada corta una línea
          // continua para que lea como espuma pegada a roca, no como borde UI.
          float distOrilla = min(vUv.x, 1.0 - vUv.x) * 2.0;
          float bandaOrilla = 1.0 - smoothstep(0.02, 0.24, distOrilla);
          float granoOrilla = campoFlujo(q * vec2(2.7, 1.9) + 31.7, t * 1.45);
          float contactoOrilla = bandaOrilla * smoothstep(0.34, 0.72, granoOrilla);
          m = max(m, contactoOrilla * uEspumaOrilla * (0.64 + rap * 0.26));
          ...
          col += vec3(1.22, 1.18, 1.05) * (spec * 0.55 + glint * 1.1 + brilloVeta * 0.72);
    ...
    // UniformsUtils.merge clona los uniforms; conservamos estas referencias
    // compartidas para que el retorno `efectos` los controle en caliente.
    mat.uniforms.uTiempo = uniforms.uTiempo;
    mat.uniforms.uFlujoDireccional = uniforms.uFlujoDireccional;
    mat.uniforms.uVetas = uniforms.uVetas;
    mat.uniforms.uEspumaOrilla = uniforms.uEspumaOrilla;
  ...
  return {
    grupo, tick, contarTriangulos, curva, enCurso, profundidadEn,
    efectos: {
      flujoDireccional: uniforms.uFlujoDireccional,
      vetas: uniforms.uVetas,
      espumaOrilla: uniforms.uEspumaOrilla,
    },
  };
}
 ...
 *   cortes    {Array<number>} cortes normalizados, de labio (1) a pie (0)
 *   alturaT   {(t)=>number} altura local del perfil, para seguir repisas
 *   anchoFinal {number} ancho relativo del último salto (def 0.85)
...
export function crearChorrera(THREE, opts = {}) {
  ...
  const solDir = (opts.solDir ? opts.solDir.clone() : new THREE.Vector3(0.44, 0.66, 0.38)).normalize();
  const alturaT = opts.alturaT ?? ((t) => ALTO * t);
  ...
  const cortes = [];
  if (Array.isArray(opts.cortes) && opts.cortes.length === SALTOS + 1) {
    for (const corte of opts.cortes) cortes.push(Math.min(1, Math.max(0, corte)));
  } else {
  ...
  function rutaCaida(tTop, tBot, anchos, SEGS) {
    ...
    for (let i = 0; i <= SEGS; i++) {
      ...
      const c = caraEn(t);
      muestras.push({ x: c.x + derivaEn(t), y: alturaT(t), z: c.z, t });
    }
  ...
  for (let s = 0; s < SALTOS; s++) {
    ...
    const relFinal = opts.anchoFinal ?? 0.85;
    const segW = ANCHO * (SALTOS < 3 ? 1 : ultimo ? relFinal : s === Math.floor(SALTOS / 2) ? 1.2 : 0.85);
    ...
    const cB = caraEn(tB);
    anclasBruma.push({
      x: cB.x + derivaEn(tB), y: alturaT(tB) + (ultimo ? 0.35 : 0.6), z: cB.z + 0.5,
