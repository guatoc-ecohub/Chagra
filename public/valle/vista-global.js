...
export function initVistaGlobal() {
  ...
  const rockMat = new THREE.MeshStandardMaterial({ color: 0x707a83, roughness: 1, flatShading: true });
  function terraza(i) {
    ...
    for (let k = 0; k < 9; k++) {
      const rk = new THREE.Mesh(new THREE.DodecahedronGeometry(rand(0.8, 2.2), 0), rockMat);
      rk.position.set(rand(-ANCHO / 2 + 6, ANCHO / 2 - 6), y - rand(0.2, 1.4), zF - rand(-0.5, 2.5));
      rk.rotation.set(rand(0, 3), rand(0, 3), rand(0, 3)); rk.scale.y = rand(0.6, 1); raiz.add(rk);
    }
    return { y, zF, zB };
  }
  ...
  // FRAILEJÓN (DR-paramo): tallo con necromasa + roseta velluda en espiral
  function frailejon(esc = 1) {
    const g = new THREE.Group();
    const tallo = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.64, 3.4, 8),
      new THREE.MeshStandardMaterial({ color: 0x6f5f42, roughness: 1, flatShading: true }));
    tallo.position.y = 1.7; g.add(tallo);
    for (let k = 0; k < 4; k++) {
      const aro = new THREE.Mesh(new THREE.ConeGeometry(0.9 - k * 0.05, 0.72, 9, 1, true),
        new THREE.MeshStandardMaterial({ color: 0x8a7550, roughness: 1, side: THREE.DoubleSide, flatShading: true }));
      aro.position.y = 0.7 + k * 0.7; aro.rotation.x = Math.PI; g.add(aro);
    }
    const roseta = new THREE.Group(); roseta.position.y = 3.5;
    for (let k = 0; k < 12; k++) {
      const a = (k / 12) * Math.PI * 2;
      const h = new THREE.Mesh(new THREE.ConeGeometry(0.17, 1.2, 4),
        new THREE.MeshStandardMaterial({ color: 0xaec19a, roughness: 1, flatShading: true }));
      h.position.set(Math.cos(a) * 0.5, 0.2, Math.sin(a) * 0.5);
      h.rotation.z = -Math.cos(a) * 0.95; h.rotation.x = Math.sin(a) * 0.95; roseta.add(h);
    }
    g.add(roseta); g.scale.setScalar(esc); return g;
  }
  function pajonal() {
    const g = new THREE.Group();
    for (let k = 0; k < 10; k++) {
      const a = Math.random() * Math.PI * 2, r = Math.random() * 0.55;
      const b = new THREE.Mesh(new THREE.ConeGeometry(0.09, rand(1.1, 2.0), 3),
        new THREE.MeshStandardMaterial({ color: 0xcdb877, roughness: 1, flatShading: true }));
      b.position.set(Math.cos(a) * r, 0.85, Math.sin(a) * r); b.rotation.z = (Math.random() - 0.5) * 0.5; g.add(b);
    }
    return g;
  }
