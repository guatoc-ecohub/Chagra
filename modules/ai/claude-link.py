...
# Mocks and simple setups for the required endpoints
...
# For writing, we'd need to modify the actual non-run file, but we will assume an API or sops execution.
# In a real environment, modifying SOPS from an API requires the age key.
...
def claude_link_get(uuid: str, token: str):
    ...
    expected_token = hmac.new(SECRET_KEY.encode(), uuid.encode(), hashlib.sha256).hexdigest()
